...
Azure Service Operator supports four different styles of authentication today. Each of these options can be used either as a global credential applied to all resources created by the operator (as shown below), or as a per-resource or per-namespace credential as documented in [single-operator-multitenancy](https://azure.github.io/azure-service-operator/guide/multitenant-deployment/#single-operator-multitenancy-default).
