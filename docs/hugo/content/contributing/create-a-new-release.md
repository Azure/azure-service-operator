...
4. Generate release notes using the `create-release-notes` skill (in `.agents/skills/create-release-notes/SKILL.md`), or manually by running:

   ``` bash
   scripts/v2/generate-raw-changelog.sh
   ```

   Then format the raw changelog into polished release notes with sections for New resources, Features, Bug fixes, and Documentation. Save the result to a file (e.g. `/tmp/aso-v2.20.0-release-notes.md`).

5. Create a draft release using the release script:

   ``` bash
   scripts/v2/create-release.sh <VERSION> <RELEASE_NOTES_FILE>
   ```

   For example: `scripts/v2/create-release.sh v2.20.0 /tmp/aso-v2.20.0-release-notes.md`. This creates a draft release on GitHub targeting `main`.

6. Review and publish the draft release. This will automatically trigger a GitHub action to build and publish an updated Docker image with the latest manager changes.
7. Ensure that the action associated with your release finishes successfully.
8. [Update our documentation](#update-resource-documentation) to move resources listed under "Next Release" to the heading "Released".
9. [Update the ROADMAP](#update-roadmap) to reflect the new release.
10. [Update Breaking Changes](#update-breaking-changes) if there are any.
11. [Create a blog post](#create-a-blog-post) highlighting the key aspects of the release.
12. Post a message in the Slack channel.
