...
type ControllerFactory struct {
	ClientCreator   func(eventhubs.EventHubManager, logr.Logger, record.EventRecorder) ResourceManager
...
func (factory *ControllerFactory) create(kubeClient client.Client, logger logr.Logger, recorder record.EventRecorder, parameters reconciler.ReconcileParameters) (*reconciler.GenericController, error) {
	...
	// create a CompletionRunner for writing secret
	secretsWriterFactory := func(c *reconciler.GenericController) reconciler.CompletionRunner {
	...
	return reconciler.CreateGenericController(parameters, ResourceKind, kubeClient, logger, recorder, factory.Scheme, &resourceManagerClient, &definitionManager{}, FinalizerName, shared.AnnotationBaseName, secretsWriterFactory)
}
	...
	return &reconciler.DependencyDefinitions{
		...
		Owner: shared.GetOwnerIfManaged(ehnInstance.ObjectMeta, func() *reconciler.Dependency {
			return &reconciler.Dependency{
				...
				SucceededAccessor: reconciler.AsSuccessAccessor(eventhubnamespaceaccessors.GetStatus),
			}
		}),
	}, nil
}
