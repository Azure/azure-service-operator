...
2. **GVK derived.** Convert Azure type + version to Kubernetes GVK:
   ...
   - **version** — `v` prefix + `api-version` with non-alphanumeric characters removed. `2021-06-01` → `v20210601`; `2021-04-01-preview` → `v20210401preview`.
...
Within the group file, add the resource under the correct version, keeping versions in numerical order:
...
- **Passwords/keys typed as `string`.** Anything a user supplies as a secret must become a `genruntime.SecretReference`. Mark it in config with `$isSecret: true` on the property. The Azure spec usually annotates these with `x-ms-secret`, but not always.
...
Every new resource needs a hand-written CRUD test — even new versions of existing resources. Copy an existing test from `v2/internal/controllers/` that targets a similar resource. Naming is defined by the README in that folder as `<group>_<subject>[_<scenario>]_<version>_test.go` (e.g. `containerregistry_registry_20230701_test.go`, `networking_virtualnetwork_v20201101_test.go`) — **always follow this pattern**, even though some existing tests do not. 
...
The group segment is title-cased using `cases.Title(language.English)` — only the first letter changes (`synapse` → `Synapse`, `appconfiguration` → `Appconfiguration`). For dotted groups like `network.frontdoor`, each dot-separated segment is title-cased independently.
...
- **"$export specified for type X but not consumed"** — configuration you added had no effect. Usually a typo: the error prints `did you mean ...?` with the nearest match. If your target is a preview version, you also need a `typeFilters` entry — preview versions are pruned by default. Add an `include` filter **before** the `prune` filter, and give a reason:
