...
```bash
source test.env && ./hack/tools/task controller:test-samples
```
...
* If recordings for all tests exist, playback will complete in about 5-10 minutes of actual test time (plus build/codegen overhead). Do not interrupt the test, just wait for it to complete.

* If any recordings are missing, each will result in a live test automatically run against Azure to create the missing recording. For most resources, this will take 20-30 minutes, but for some resources a recording may take 60m or more.
...
After you have successfully updated a recording, run the test again (same command, **without** `source test.env`) to verify that playback works correctly. Playback is much faster than recording — typically 2-3 minutes. If playback fails but recording succeeded, this indicates a systemic issue with the VCR infrastructure (e.g. non-deterministic request serialization), NOT a stale recording. Do not keep re-recording — investigate and report to the user.
...
1. **Partial recording files**: A failed test may write a partial recording file. Always delete this file before retrying.
...
Why do this? A test run winds up as soon as one test fails, leaving later tests unexecuted. By starting over and running all the tests, you can confirm that all problems are fully resolved and that there are no other hidden issues.
...
- Playback-only (full suite): 10 minutes (600000ms) — total is 5-10 minutes of test time plus build/codegen overhead
- Playback-only (single test): 5 minutes (300000ms) — typically completes in 2-3 minutes
