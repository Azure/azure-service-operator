...
if __name__ == '__main__':
    frontMatterRegex = r'(?s)^---\s*(.*)\s*---$'
    regex = re.compile(frontMatterRegex, re.MULTILINE | re.IGNORECASE)
    failed_files = []

    print_colorful('==> Checking Markdown frontmatter <==')
    ...
    if failed_files:
        print('The following files are missing the required frontmatter:')
        for name in failed_files:
            print(f'\t{name}')

    print_colorful('Done checking Markdown frontmatter')
