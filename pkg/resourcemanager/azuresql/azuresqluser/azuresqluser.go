...
func (s *AzureSqlUserManager) Ensure(ctx context.Context, obj runtime.Object, opts ...resourcemanager.ConfigOption) (bool, error) {
	...
	if err != nil {
		...
		instance.Status.Message = fmt.Sprintf("admin secret : %s, not found in %T", key.String(), adminSecretClient)
		return false, nil
	}
	...
	var keyVaultEnabled bool
	switch sqlUserSecretClient.(type) {
	case *keyvault.KeyvaultSecretClient:
