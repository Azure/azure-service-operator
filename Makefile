...
test-existing-controllers: generate fmt vet manifests
	TEST_USE_EXISTING_CLUSTER=true TEST_CONTROLLER_WITH_MOCKS=false REQUEUE_AFTER=20 go test -tags all -parallel 3 -v -coverprofile=coverage-existing.txt -covermode count ./controllers/... -timeout 30m 2>&1 | tee testlogs-existing.txt
...
test-existing-managers: generate fmt vet manifests
	TEST_USE_EXISTING_CLUSTER=true TEST_CONTROLLER_WITH_MOCKS=false REQUEUE_AFTER=20 go test -v -coverprofile=coverage-existing.txt -covermode count ./api/... ./pkg/resourcemanager/sqlclient/... ./pkg/resourcemanager/eventhubs/...  ./pkg/resourcemanager/resourcegroups/...  ./pkg/resourcemanager/storages/... ./pkg/resourcemanager/psql/server/... ./pkg/resourcemanager/psql/database/... ./pkg/resourcemanager/psql/firewallrule/... -timeout 40m 2>&1
