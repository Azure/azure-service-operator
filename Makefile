...
test: generate fmt vet manifests
	TEST_USE_EXISTING_CLUSTER=false ginkgo -nodes=4 -stream -progress -cover -outputdir $(pwd) -coverprofile coverage.txt -covermode count ./api/... ./controllers/... ./pkg/resourcemanager/eventhubs/...  ./pkg/resourcemanager/resourcegroups/...  ./pkg/resourcemanager/storages/... 2>&1 | tee testlogs.txt
	go-junit-report < testlogs.txt  > report.xml
	go tool cover -html=coverage.txt -o cover.html
# Run tests with existing cluster
test-existing: generate fmt vet manifests
	TEST_USE_EXISTING_CLUSTER=true ginkgo -nodes=4 -stream -progress -cover -outputdir $(pwd) -coverprofile=coverage-existing.txt -covermode count ./api/... ./controllers/... ./pkg/resourcemanager/eventhubs/...  ./pkg/resourcemanager/resourcegroups/...  ./pkg/resourcemanager/storages/... 2>&1 | tee testlogs-existing.txt
