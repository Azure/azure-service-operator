...
var _ = Describe("PostgreSQLFirewallRule Controller", func() {
	...
	Context("Create and Delete", func() {
		It("should create and delete PostgreSQL firewall rule", func() {
			defer GinkgoRecover()
			...
			err = tc.k8sClient.Create(context.Background(), postgreSQLFirewallRuleInstance)
			Expect(apierrors.IsInvalid(err)).To(Equal(false))
			Expect(err).NotTo(HaveOccurred())
			...
			Eventually(func() bool {
				_ = tc.k8sClient.Get(context.Background(), postgreSQLFirewallRuleNamespacedName, postgreSQLFirewallRuleInstance)
			...
			err = tc.k8sClient.Delete(context.Background(), postgreSQLFirewallRuleInstance)
			Expect(err).NotTo(HaveOccurred())

			Eventually(func() bool {
				err = tc.k8sClient.Get(context.Background(), postgreSQLFirewallRuleNamespacedName, postgreSQLFirewallRuleInstance)
				return err != nil
			}, tc.timeout, tc.retry,
			).Should(BeTrue())
