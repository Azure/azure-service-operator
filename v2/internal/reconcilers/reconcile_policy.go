...
func ParseReconcilePolicy(policy string, defaultReconcilePolicy string) (annotations.ReconcilePolicyValue, error) {
	// policy is read from CR annotation, if it's empty it being read from defaultReconcilePolicy
	switch policy {
	case "":
		return annotations.ReconcilePolicyValue(defaultReconcilePolicy), nil
