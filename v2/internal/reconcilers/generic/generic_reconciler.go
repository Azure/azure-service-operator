...
func (gr *GenericReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	log.V(Info).Info("Object to reconcile", metaObj)
...
func (gr *GenericReconciler) mergeReconcilePolicy(ctx context.Context, log logr.Logger, obj genruntime.MetaObject) annotations.ReconcilePolicyValue {
	log.V(Info).Info("Looking for namespace annotations")
	// Claim resource namespace
	// We initially get the reconcile policy from the object itself
	reconcilePolicy, err := reconcilers.GetReconcilePolicy(obj, log, gr.Config.DefaultReconcilePolicy)

	// If the policy is not defined at object level, then we check if it's defined at namespace level
	// in case this is not defined, reconcilePolicy will be set to defaultReconcilePolicy and we'll have an error here
	if err != nil {
		// here we check if any annotation is specified at namespace level
		...
		// if we cannot get the namespace, we return defaul reconcile policy
		if err != nil {
			// reconcilePolicy here will be set to manage in case we get an error
			log.V(Info).Info("Error while retrieving namespace object", "error", err)
			return reconcilePolicy
		}
		log.V(Info).Info("Found Namespace annotations", namespaceObject.GetAnnotations()[annotations.ReconcilePolicy])
		namespaceAnnotation := namespaceObject.GetAnnotations()[annotations.ReconcilePolicy]
		// this will be either the reconcile policy defined at namespace level either the default one
		reconcilePolicy, err = reconcilers.ParseReconcilePolicy(namespaceAnnotation, gr.Config.DefaultReconcilePolicy)
	}

	log.V(Info).Info("Retrieved reconcile policy", reconcilePolicy)
	return reconcilePolicy
	// Here we have an annotation that comes from the namespace
}
