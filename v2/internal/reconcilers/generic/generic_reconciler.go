...
func (gr *GenericReconciler) mergeReconcilePolicy(ctx context.Context, log logr.Logger, obj genruntime.MetaObject) annotations.ReconcilePolicyValue {
	log.V(Verbose).Info("Looking for namespace annotations")
	...
	if err != nil {
		...
		if err != nil {
			...
			log.V(Verbose).Error(err, "Error while retrieving namespace object", "error", err)
			return reconcilePolicy
		}
		log.V(Verbose).Info("Found Namespace annotations", namespaceObject.GetAnnotations()[annotations.ReconcilePolicy])
	...
	log.V(Verbose).Info("Retrieved reconcile policy", reconcilePolicy)
	return reconcilePolicy
