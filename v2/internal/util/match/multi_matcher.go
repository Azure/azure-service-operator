...
func newMultiMatcher(matcher string) (StringMatcher, error) {
	splits := strings.Split(matcher, ";")
	matchers := make([]StringMatcher, 0, len(splits))
	for _, str := range splits {
		m, err := NewStringMatcher(str)
		if err != nil {
			return nil, err
		}
...
func (mm *multiMatcher) Matches(value string) bool {
	matches, _ := mm.MatchesDetailed(value)
	return matches
}
...
func (mm *multiMatcher) MatchesDetailed(value string) (bool, string) {
	result := false
	details := ""
	...
	for _, m := range mm.matchers {
		matches, d := m.MatchesDetailed(value)
		if matches {
			result = true
