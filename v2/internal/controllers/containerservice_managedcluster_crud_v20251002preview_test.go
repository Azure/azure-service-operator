...
func Test_AKS_ManagedCluster_20251002preview_CRUD(t *testing.T) {
	t.Parallel()
	...
	skuName := aks.ManagedClusterSKUName_Base
	skuTier := aks.ManagedClusterSKUTier_Standard
	...
	tc.Expect(*cluster.Status.Sku.Name).To(Equal(aks.ManagedClusterSKUName_STATUS_Base))
	tc.Expect(cluster.Status.Sku.Tier).ToNot(BeNil())
	tc.Expect(*cluster.Status.Sku.Tier).To(Equal(aks.ManagedClusterSKUTier_STATUS_Standard))
	...
	tc.RunParallelSubtests(
		testcommon.Subtest{
			...
			Test: func(tc *testcommon.KubePerTestContext) {
				AKS_ManagedCluster_AgentPool_20251002preview_CRUD(tc, cluster)
			},
		},
		testcommon.Subtest{
...
func AKS_ManagedCluster_AgentPool_20251002preview_CRUD(tc *testcommon.KubePerTestContext, cluster *aks.ManagedCluster) {
...
func AKS_ManagedCluster_Kubeconfig_20251002preview_Secrets(tc *testcommon.KubePerTestContext, cluster *aks.ManagedCluster) {
	old := cluster.DeepCopy()
	secret := "kubeconfig"
...
func AKS_ManagedCluster_MaintenanceConfiguration_20251002preview_CRUD(tc *testcommon.KubePerTestContext, cluster *aks.ManagedCluster) {
