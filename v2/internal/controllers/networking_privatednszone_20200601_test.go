...
func Test_Networking_PrivateDnsZone_20200601_CRUD(t *testing.T) {
	t.Parallel()
	...
	zone := newPrivateDNSZone20200601(tc, name, rg)
	...
	tc.RunParallelSubtests(
		testcommon.Subtest{
			...
			Test: func(tc *testcommon.KubePerTestContext) {
				PrivateDNSZone_VirtualNetworkLinks_20200601_CRUD(tc, zone, rg)
			},
		},
		testcommon.Subtest{
			Name: "Test_Recordset_CRUD",
			Test: func(tc *testcommon.KubePerTestContext) {
				PrivateDNSZones_CNAME_Record_20200601_Test(tc, zone)
			},
		},
	)
...
func PrivateDNSZones_CNAME_Record_20200601_Test(tc *testcommon.KubePerTestContext, zone *network.PrivateDnsZone) {
...
func PrivateDNSZone_VirtualNetworkLinks_20200601_CRUD(tc *testcommon.KubePerTestContext, zone *network.PrivateDnsZone, rg *resources.ResourceGroup) {
	vnet := newVMVirtualNetwork(tc, testcommon.AsOwner(rg))
	link := newVirtualNetworkLink20200601(tc, zone, vnet)
...
func newPrivateDNSZone20200601(tc *testcommon.KubePerTestContext, name string, rg *resources.ResourceGroup) *network.PrivateDnsZone {
...
func newVirtualNetworkLink20200601(tc *testcommon.KubePerTestContext, dnsZone *network.PrivateDnsZone, vnet *network20201101.VirtualNetwork) *network20200601.PrivateDnsZonesVirtualNetworkLink {
