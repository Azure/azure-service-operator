...
import (
	...
	cache "github.com/Azure/azure-service-operator/v2/api/cache/v20250701"
...
func Test_Cache_RedisEnterprise_20250701_CRUD(t *testing.T) {
	t.Parallel()
	tc := globalTestContext.ForTest(t)

	// Create a resource group for the test
	rg := tc.CreateTestResourceGroupAndWait()
	tls12 := cache.ClusterCreateProperties_MinimumTlsVersion_12
	highAvailability := cache.ClusterCreateProperties_HighAvailability_Enabled
	publicNetworkAccess := cache.ClusterCreateProperties_PublicNetworkAccess_Enabled
	...
	tc.Expect(*redis.Status.MinimumTlsVersion).To(Equal(cache.ClusterCreateProperties_MinimumTlsVersion_STATUS_12))
	tc.Expect(redis.Status.HighAvailability).ToNot(BeNil())
	tc.Expect(*redis.Status.HighAvailability).To(Equal(cache.ClusterCreateProperties_HighAvailability_STATUS_Enabled))
	tc.Expect(redis.Status.PublicNetworkAccess).ToNot(BeNil())
	tc.Expect(*redis.Status.PublicNetworkAccess).To(Equal(cache.ClusterCreateProperties_PublicNetworkAccess_STATUS_Enabled))
	...
	tc.RunSubtests(
		testcommon.Subtest{
			...
			Test: func(tc *testcommon.KubePerTestContext) {
				RedisEnterprise_Database_20250701_CRUD(tc, &redis)
			},
		},
		testcommon.Subtest{
			Name: "RedisEnterprise database access policy assignment CRUD",
			Test: func(tc *testcommon.KubePerTestContext) {
				RedisEnterprise_Database_AccessPolicyAssignment_20250701_CRUD(tc, rg, &redis)
			},
		},
	)
...
func RedisEnterprise_Database_20250701_CRUD(tc *testcommon.KubePerTestContext, redis *cache.RedisEnterprise) {
	...
	tc.Expect(to.Value(db.Status.ClusteringPolicy)).To(Equal(cache.DatabaseCreateProperties_ClusteringPolicy_STATUS_OSSCluster))
	armId := *db.Status.Id
...
func RedisEnterprise_Database_AccessPolicyAssignment_20250701_CRUD(tc *testcommon.KubePerTestContext, rg *resources.ResourceGroup, redis *cache.RedisEnterprise) {
	...
	accessKeysAuthenticationDisabled := cache.DatabaseCreateProperties_AccessKeysAuthentication_Disabled
	...
	tc.Expect(*db.Status.AccessKeysAuthentication).To(Equal(cache.DatabaseCreateProperties_AccessKeysAuthentication_STATUS_Disabled))
