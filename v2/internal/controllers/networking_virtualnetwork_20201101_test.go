...
func Test_Networking_VirtualNetwork_20201101_CRUD(t *testing.T) {
	t.Parallel()
...
func Subnet_20201101_CRUD(tc *testcommon.KubePerTestContext, vnet *network.VirtualNetwork) {
...
func Test_Networking_Subnet_CreatedThenVNETUpdated_20201101_SubnetStillExists(t *testing.T) {
	t.Parallel()
	tc := globalTestContext.ForTest(t)
	rg := tc.CreateTestResourceGroupAndWait()

	vnet := newVNet20201101(tc, testcommon.AsOwner(rg), []string{"10.0.0.0/16"})
...
func Test_Networking_VirtualNetworkAndSubnetAdopted_20201101_SubnetsStillExist(t *testing.T) {
	t.Parallel()
