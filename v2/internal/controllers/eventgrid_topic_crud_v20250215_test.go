...
func Test_EventGrid_Topic_CRUD_20250215(t *testing.T) {
	t.Parallel()
	...
	tc.Expect(topic.Status.DisableLocalAuth).To(HaveValue(BeTrue()))
	...
	tc.RunParallelSubtests(
		testcommon.Subtest{
			...
			Test: func(tc *testcommon.KubePerTestContext) {
				Topic_Subscription_CRUD_20250215(tc, rg, topic)
			},
		},
		testcommon.Subtest{
			Name: "Topic_SecretsWrittenToSameKubeSecret",
			Test: func(tc *testcommon.KubePerTestContext) {
				Topic_SecretsWrittenToSameKubeSecret_20250215(tc, topic)
			},
		},
	)
...
func Topic_SecretsWrittenToSameKubeSecret_20250215(tc *testcommon.KubePerTestContext, topic *eventgrid.Topic) {
	old := topic.DeepCopy()
...
func Topic_Subscription_CRUD_20250215(tc *testcommon.KubePerTestContext, rg *resources.ResourceGroup, topic *eventgrid.Topic) {
