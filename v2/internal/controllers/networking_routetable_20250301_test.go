...
func Test_Networking_RouteTable_20250301_CRUD(t *testing.T) {
	t.Parallel()
	...
	tc.RunParallelSubtests(
		testcommon.Subtest{
			...
			Test: func(tc *testcommon.KubePerTestContext) {
				Routes_20250301_CRUD(tc, routeTable)
			},
		},
	)
...
func Routes_20250301_CRUD(tc *testcommon.KubePerTestContext, routeTable *network.RouteTable) {
...
func Test_Networking_Route_CreatedThenRouteTableUpdated_20250301_RouteStillExists(t *testing.T) {
	t.Parallel()
	tc := globalTestContext.ForTest(t)
	rg := tc.CreateTestResourceGroupAndWait()
