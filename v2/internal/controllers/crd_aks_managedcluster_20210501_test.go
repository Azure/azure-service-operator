...
import (
	...
	aks "github.com/Azure/azure-service-operator/v2/api/containerservice/v1api20210501"
	"github.com/Azure/azure-service-operator/v2/internal/testcommon"
	"github.com/Azure/azure-service-operator/v2/pkg/genruntime"
)

func Test_AKS_ManagedCluster_20210501_CRUD(t *testing.T) {
	t.Parallel()
	...
	tc.RunSubtests(
		testcommon.Subtest{
			...
			Test: func(tc *testcommon.KubePerTestContext) {
				AKS_ManagedCluster_Kubeconfig_20210501_Secrets(tc, cluster)
			},
		})
	tc.RunParallelSubtests(
		testcommon.Subtest{
			...
			Test: func(tc *testcommon.KubePerTestContext) {
				AKS_ManagedCluster_AgentPool_20210501_CRUD(tc, cluster)
			},
		},
	)
...
func AKS_ManagedCluster_AgentPool_20210501_CRUD(tc *testcommon.KubePerTestContext, cluster *aks.ManagedCluster) {
	osType := aks.OSType_Linux
	agentPoolMode := aks.AgentPoolMode_System
...
func AKS_ManagedCluster_Kubeconfig_20210501_Secrets(tc *testcommon.KubePerTestContext, cluster *aks.ManagedCluster) {
	old := cluster.DeepCopy()
	secret := "kubeconfig"
