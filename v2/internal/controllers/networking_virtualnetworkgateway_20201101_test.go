...
func Test_Networking_VirtualNetworkGateway_20201101_CRUD(t *testing.T) {
	t.Parallel()
	...
	gateway := newVnetGateway20201101(tc, publicIPAddress, subnet, rg)

	tc.CreateResourceAndWait(gateway)
	tc.DeleteResourceAndWait(gateway)
}

func newVnetGateway20201101(tc *testcommon.KubePerTestContext, publicIPAddress *network.PublicIPAddress, subnet *network.VirtualNetworksSubnet, rg *resources.ResourceGroup) *network.VirtualNetworkGateway {
