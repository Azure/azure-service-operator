...
func TestRecorder_WhenRecordingAndRecordingExists_DoesPlayback(t *testing.T) {
	...
	exists, err := vcr.CassetteFileExists(cassetteName)
	g.Expect(err).To(BeNil())
	g.Expect(exists).To(BeTrue()) // Comment out if rerecording, see (ii) above

	// Create our TestRecorder and ensure it's in replay mode
	recorder, err := NewTestRecorder(cassetteName, cfg, logr.Discard())
	g.Expect(err).To(BeNil())
	g.Expect(recorder.IsReplaying()).To(BeTrue()) // Comment out if rerecording, see (ii) above
