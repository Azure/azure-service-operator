...
func Test_SqlDatabase_Spec_ARM_WhenSerializedToJson_DeserializesAsEqual(t *testing.T) {
	t.Parallel()
	parameters := gopter.DefaultTestParameters()
	...
	properties := gopter.NewProperties(parameters)
	properties.Property(
		"Round trip of SqlDatabase_Spec_ARM via JSON returns original",
		prop.ForAll(RunJSONSerializationTestForSqlDatabase_Spec_ARM, SqlDatabase_Spec_ARMGenerator()))
	properties.TestingRun(t, gopter.NewFormatedReporter(true, 240, os.Stdout))
}

// RunJSONSerializationTestForSqlDatabase_Spec_ARM runs a test to see if a specific instance of SqlDatabase_Spec_ARM round trips to JSON and back losslessly
func RunJSONSerializationTestForSqlDatabase_Spec_ARM(subject SqlDatabase_Spec_ARM) string {
	// Serialize to JSON
	bin, err := json.Marshal(subject)
	if err != nil {
		return err.Error()
	...
	// Deserialize back into memory
	var actual SqlDatabase_Spec_ARM
	err = json.Unmarshal(bin, &actual)
	if err != nil {
		return err.Error()
	}
...
// Generator of SqlDatabase_Spec_ARM instances for property testing - lazily instantiated by
// SqlDatabase_Spec_ARMGenerator()
var sqlDatabase_Spec_ARMGenerator gopter.Gen

// SqlDatabase_Spec_ARMGenerator returns a generator of SqlDatabase_Spec_ARM instances for property testing.
// We first initialize sqlDatabase_Spec_ARMGenerator with a simplified generator based on the
// fields with primitive types then replacing it with a more complex one that also handles complex fields
// to ensure any cycles in the object graph properly terminate.
func SqlDatabase_Spec_ARMGenerator() gopter.Gen {
	if sqlDatabase_Spec_ARMGenerator != nil {
		return sqlDatabase_Spec_ARMGenerator
	}

	generators := make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForSqlDatabase_Spec_ARM(generators)
	sqlDatabase_Spec_ARMGenerator = gen.Struct(reflect.TypeOf(SqlDatabase_Spec_ARM{}), generators)

	// The above call to gen.Struct() captures the map, so create a new one
	generators = make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForSqlDatabase_Spec_ARM(generators)
	AddRelatedPropertyGeneratorsForSqlDatabase_Spec_ARM(generators)
	sqlDatabase_Spec_ARMGenerator = gen.Struct(reflect.TypeOf(SqlDatabase_Spec_ARM{}), generators)

	return sqlDatabase_Spec_ARMGenerator
}

// AddIndependentPropertyGeneratorsForSqlDatabase_Spec_ARM is a factory method for creating gopter generators
func AddIndependentPropertyGeneratorsForSqlDatabase_Spec_ARM(gens map[string]gopter.Gen) {
...
// AddRelatedPropertyGeneratorsForSqlDatabase_Spec_ARM is a factory method for creating gopter generators
func AddRelatedPropertyGeneratorsForSqlDatabase_Spec_ARM(gens map[string]gopter.Gen) {
