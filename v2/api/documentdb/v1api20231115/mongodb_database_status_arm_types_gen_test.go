...
func Test_MongodbDatabase_STATUS_ARM_WhenSerializedToJson_DeserializesAsEqual(t *testing.T) {
	t.Parallel()
	parameters := gopter.DefaultTestParameters()
	parameters.MinSuccessfulTests = 80
	parameters.MaxSize = 3
	properties := gopter.NewProperties(parameters)
	properties.Property(
		"Round trip of MongodbDatabase_STATUS_ARM via JSON returns original",
		prop.ForAll(RunJSONSerializationTestForMongodbDatabase_STATUS_ARM, MongodbDatabase_STATUS_ARMGenerator()))
	properties.TestingRun(t, gopter.NewFormatedReporter(true, 240, os.Stdout))
}

// RunJSONSerializationTestForMongodbDatabase_STATUS_ARM runs a test to see if a specific instance of MongodbDatabase_STATUS_ARM round trips to JSON and back losslessly
func RunJSONSerializationTestForMongodbDatabase_STATUS_ARM(subject MongodbDatabase_STATUS_ARM) string {
	// Serialize to JSON
	bin, err := json.Marshal(subject)
	if err != nil {
		return err.Error()
	...
	// Deserialize back into memory
	var actual MongodbDatabase_STATUS_ARM
	err = json.Unmarshal(bin, &actual)
	if err != nil {
		return err.Error()
	}
...
// Generator of MongodbDatabase_STATUS_ARM instances for property testing - lazily instantiated by
// MongodbDatabase_STATUS_ARMGenerator()
var mongodbDatabase_STATUS_ARMGenerator gopter.Gen

// MongodbDatabase_STATUS_ARMGenerator returns a generator of MongodbDatabase_STATUS_ARM instances for property testing.
// We first initialize mongodbDatabase_STATUS_ARMGenerator with a simplified generator based on the
// fields with primitive types then replacing it with a more complex one that also handles complex fields
// to ensure any cycles in the object graph properly terminate.
func MongodbDatabase_STATUS_ARMGenerator() gopter.Gen {
	if mongodbDatabase_STATUS_ARMGenerator != nil {
		return mongodbDatabase_STATUS_ARMGenerator
	}

	generators := make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForMongodbDatabase_STATUS_ARM(generators)
	mongodbDatabase_STATUS_ARMGenerator = gen.Struct(reflect.TypeOf(MongodbDatabase_STATUS_ARM{}), generators)

	// The above call to gen.Struct() captures the map, so create a new one
	generators = make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForMongodbDatabase_STATUS_ARM(generators)
	AddRelatedPropertyGeneratorsForMongodbDatabase_STATUS_ARM(generators)
	mongodbDatabase_STATUS_ARMGenerator = gen.Struct(reflect.TypeOf(MongodbDatabase_STATUS_ARM{}), generators)

	return mongodbDatabase_STATUS_ARMGenerator
}

// AddIndependentPropertyGeneratorsForMongodbDatabase_STATUS_ARM is a factory method for creating gopter generators
func AddIndependentPropertyGeneratorsForMongodbDatabase_STATUS_ARM(gens map[string]gopter.Gen) {
	gens["Id"] = gen.PtrOf(gen.AlphaString())
...
// AddRelatedPropertyGeneratorsForMongodbDatabase_STATUS_ARM is a factory method for creating gopter generators
func AddRelatedPropertyGeneratorsForMongodbDatabase_STATUS_ARM(gens map[string]gopter.Gen) {
