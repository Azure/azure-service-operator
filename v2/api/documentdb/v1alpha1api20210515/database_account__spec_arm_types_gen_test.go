...
func AddRelatedPropertyGeneratorsForDatabaseAccount_SpecARM(gens map[string]gopter.Gen) {
	...
	gens["Properties"] = DatabaseAccountPropertiesARMGenerator()
}

func Test_DatabaseAccountPropertiesARM_WhenSerializedToJson_DeserializesAsEqual(t *testing.T) {
	t.Parallel()
	parameters := gopter.DefaultTestParameters()
	parameters.MaxSize = 10
	properties := gopter.NewProperties(parameters)
	properties.Property(
		"Round trip of DatabaseAccountPropertiesARM via JSON returns original",
		prop.ForAll(RunJSONSerializationTestForDatabaseAccountPropertiesARM, DatabaseAccountPropertiesARMGenerator()))
	properties.TestingRun(t, gopter.NewFormatedReporter(true, 240, os.Stdout))
}

// RunJSONSerializationTestForDatabaseAccountPropertiesARM runs a test to see if a specific instance of DatabaseAccountPropertiesARM round trips to JSON and back losslessly
func RunJSONSerializationTestForDatabaseAccountPropertiesARM(subject DatabaseAccountPropertiesARM) string {
	// Serialize to JSON
	bin, err := json.Marshal(subject)
	if err != nil {
		return err.Error()
	...
	// Deserialize back into memory
	var actual DatabaseAccountPropertiesARM
	err = json.Unmarshal(bin, &actual)
	if err != nil {
		return err.Error()
	}
...
// Generator of DatabaseAccountPropertiesARM instances for property testing - lazily instantiated by
//DatabaseAccountPropertiesARMGenerator()
var databaseAccountPropertiesARMGenerator gopter.Gen

// DatabaseAccountPropertiesARMGenerator returns a generator of DatabaseAccountPropertiesARM instances for property testing.
// We first initialize databaseAccountPropertiesARMGenerator with a simplified generator based on the
// fields with primitive types then replacing it with a more complex one that also handles complex fields
// to ensure any cycles in the object graph properly terminate.
func DatabaseAccountPropertiesARMGenerator() gopter.Gen {
	if databaseAccountPropertiesARMGenerator != nil {
		return databaseAccountPropertiesARMGenerator
	}

	generators := make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForDatabaseAccountPropertiesARM(generators)
	databaseAccountPropertiesARMGenerator = gen.Struct(reflect.TypeOf(DatabaseAccountPropertiesARM{}), generators)

	// The above call to gen.Struct() captures the map, so create a new one
	generators = make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForDatabaseAccountPropertiesARM(generators)
	AddRelatedPropertyGeneratorsForDatabaseAccountPropertiesARM(generators)
	databaseAccountPropertiesARMGenerator = gen.Struct(reflect.TypeOf(DatabaseAccountPropertiesARM{}), generators)

	return databaseAccountPropertiesARMGenerator
}

// AddIndependentPropertyGeneratorsForDatabaseAccountPropertiesARM is a factory method for creating gopter generators
func AddIndependentPropertyGeneratorsForDatabaseAccountPropertiesARM(gens map[string]gopter.Gen) {
...
// AddRelatedPropertyGeneratorsForDatabaseAccountPropertiesARM is a factory method for creating gopter generators
func AddRelatedPropertyGeneratorsForDatabaseAccountPropertiesARM(gens map[string]gopter.Gen) {
