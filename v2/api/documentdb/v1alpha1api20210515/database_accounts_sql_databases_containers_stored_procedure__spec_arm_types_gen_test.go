...
func AddRelatedPropertyGeneratorsForDatabaseAccountsSqlDatabasesContainersStoredProcedure_SpecARM(gens map[string]gopter.Gen) {
	gens["Properties"] = SqlStoredProcedurePropertiesARMGenerator()
}

func Test_SqlStoredProcedurePropertiesARM_WhenSerializedToJson_DeserializesAsEqual(t *testing.T) {
	t.Parallel()
	parameters := gopter.DefaultTestParameters()
	parameters.MaxSize = 10
	properties := gopter.NewProperties(parameters)
	properties.Property(
		"Round trip of SqlStoredProcedurePropertiesARM via JSON returns original",
		prop.ForAll(RunJSONSerializationTestForSqlStoredProcedurePropertiesARM, SqlStoredProcedurePropertiesARMGenerator()))
	properties.TestingRun(t, gopter.NewFormatedReporter(true, 240, os.Stdout))
}

// RunJSONSerializationTestForSqlStoredProcedurePropertiesARM runs a test to see if a specific instance of SqlStoredProcedurePropertiesARM round trips to JSON and back losslessly
func RunJSONSerializationTestForSqlStoredProcedurePropertiesARM(subject SqlStoredProcedurePropertiesARM) string {
	// Serialize to JSON
	bin, err := json.Marshal(subject)
	if err != nil {
		return err.Error()
	...
	// Deserialize back into memory
	var actual SqlStoredProcedurePropertiesARM
	err = json.Unmarshal(bin, &actual)
	if err != nil {
		return err.Error()
	}
...
// Generator of SqlStoredProcedurePropertiesARM instances for property testing - lazily instantiated by
//SqlStoredProcedurePropertiesARMGenerator()
var sqlStoredProcedurePropertiesARMGenerator gopter.Gen

// SqlStoredProcedurePropertiesARMGenerator returns a generator of SqlStoredProcedurePropertiesARM instances for property testing.
func SqlStoredProcedurePropertiesARMGenerator() gopter.Gen {
	if sqlStoredProcedurePropertiesARMGenerator != nil {
		return sqlStoredProcedurePropertiesARMGenerator
	}

	generators := make(map[string]gopter.Gen)
	AddRelatedPropertyGeneratorsForSqlStoredProcedurePropertiesARM(generators)
	sqlStoredProcedurePropertiesARMGenerator = gen.Struct(reflect.TypeOf(SqlStoredProcedurePropertiesARM{}), generators)

	return sqlStoredProcedurePropertiesARMGenerator
}

// AddRelatedPropertyGeneratorsForSqlStoredProcedurePropertiesARM is a factory method for creating gopter generators
func AddRelatedPropertyGeneratorsForSqlStoredProcedurePropertiesARM(gens map[string]gopter.Gen) {
