...
func Test_Alias_SpecARM_WhenSerializedToJson_DeserializesAsEqual(t *testing.T) {
	...
	properties.Property(
<<<<<<<< HEAD:v2/api/subscription/v1beta20211001/alias__spec_arm_types_gen_test.go
...
========
		...
		prop.ForAll(RunJSONSerializationTestForAliases_SpecARM, Aliases_SpecARMGenerator()))
	properties.TestingRun(t, gopter.NewFormatedReporter(true, 240, os.Stdout))
}

// RunJSONSerializationTestForAliases_SpecARM runs a test to see if a specific instance of Aliases_SpecARM round trips to JSON and back losslessly
func RunJSONSerializationTestForAliases_SpecARM(subject Aliases_SpecARM) string {
>>>>>>>> main:v2/api/subscription/v1beta20211001/aliases_spec_arm_types_gen_test.go
	// Serialize to JSON
...
========
// Generator of Aliases_SpecARM instances for property testing - lazily instantiated by Aliases_SpecARMGenerator()
var aliases_SpecARMGenerator gopter.Gen

// Aliases_SpecARMGenerator returns a generator of Aliases_SpecARM instances for property testing.
// We first initialize aliases_SpecARMGenerator with a simplified generator based on the
// fields with primitive types then replacing it with a more complex one that also handles complex fields
// to ensure any cycles in the object graph properly terminate.
func Aliases_SpecARMGenerator() gopter.Gen {
	if aliases_SpecARMGenerator != nil {
		return aliases_SpecARMGenerator
	}

	generators := make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForAliases_SpecARM(generators)
	aliases_SpecARMGenerator = gen.Struct(reflect.TypeOf(Aliases_SpecARM{}), generators)

	// The above call to gen.Struct() captures the map, so create a new one
	generators = make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForAliases_SpecARM(generators)
	AddRelatedPropertyGeneratorsForAliases_SpecARM(generators)
	aliases_SpecARMGenerator = gen.Struct(reflect.TypeOf(Aliases_SpecARM{}), generators)

	return aliases_SpecARMGenerator
}

// AddIndependentPropertyGeneratorsForAliases_SpecARM is a factory method for creating gopter generators
func AddIndependentPropertyGeneratorsForAliases_SpecARM(gens map[string]gopter.Gen) {
	gens["Name"] = gen.AlphaString()
}

// AddRelatedPropertyGeneratorsForAliases_SpecARM is a factory method for creating gopter generators
func AddRelatedPropertyGeneratorsForAliases_SpecARM(gens map[string]gopter.Gen) {
>>>>>>>> main:v2/api/subscription/v1beta20211001/aliases_spec_arm_types_gen_test.go
