...
type ManagedClusterProperties struct {
	...
	// SchedulerProfile: Profile with scheduler-related settings, like the configuration mode for each scheduler managed by
	// AKS. See https://aka.ms/aks/scheduler-profile.
...
type ManagedClusterAgentPoolProfile struct {
	...
	// NodeImageVersion: The version of the node image. Setting this value triggers an agentPool rollback.
...
type ManagedClusterHostedSystemProfile struct {
	...
	// NodeSubnetID: The ID of the subnet that will be joined by worker nodes managed by node auto provisioner for running
	// workload pods in your tenant. This must be provided together with `systemNodeSubnetID` and
	// `apiserverAccessProfile.subnetId`, and all three subnet IDs must be in the same VNet. If you don’t specify it, AKS
	// will create a subnet in the managed resource group using a default /16 CIDR.
	NodeSubnetID *string `json:"nodeSubnetID,omitempty"`

	// SystemNodeSubnetID: The ID of the subnet that will be joined by system nodes managed and hosted by AKS for running
	// critical system addons. This ID must be provided together with `nodeSubnetID` and `apiserverAccessProfile.subnetId`, and
	// all three subnet IDs must belong to the same VNet. If you don’t specify it, AKS will create a subnet in the managed
	// resource group using a default /26 CIDR.
	SystemNodeSubnetID *string `json:"systemNodeSubnetID,omitempty"`
}

// Cluster HTTP proxy configuration.
type ManagedClusterHTTPProxyConfig struct {
	// Enabled: Whether to enable HTTP proxy. If disabled, the specified proxy configuration will be not be set on pods and
	// nodes. If not specified, the default is true.
	Enabled *bool `json:"enabled,omitempty"`
...
type SchedulerProfile struct {
	// Upstream: Profile with settings related to upstream variant of kube-scheduler
...
type ManagedClusterAzureMonitorProfileAppMonitoring struct {
	// AutoInstrumentation: Application Monitoring auto-instrumentation for AKS. Deploys a webhook that auto-instruments
	// workloads with Microsoft OpenTelemetry Distros to collect OpenTelemetry metrics, logs, and traces. See
...
type ManagedClusterAzureMonitorProfileMetrics struct {
	// ControlPlane: Control plane metrics collection profile for the Azure Managed Prometheus addon. Configures collection of
	...
	ControlPlane *ManagedClusterAzureMonitorProfileMetricsControlPlane `json:"controlPlane,omitempty"`
...
type ManagedClusterIngressProfileWebAppRouting struct {
	...
	// GatewayAPIImplementations: Configurations for Gateway API providers to be used for managed ingress with App Routing. See
	// https://aka.ms/k8s-gateway-api for more information on the Gateway API.
	GatewayAPIImplementations *ManagedClusterWebAppRoutingGatewayAPIImplementations `json:"gatewayAPIImplementations,omitempty"`
...
type ManagedClusterSecurityProfileDefender struct {
	...
	// SecurityGating: Microsoft Defender settings for security gating. This validates container images eligibility for
	// deployment based on Defender for Containers security findings. Using Admission Controller, it either audits or prevents
	// deployment of images that do not meet security standards. For more information, see
...
type SchedulerInstanceProfile struct {
	// SchedulerConfigMode: The configuration mode to be used by the AKS-managed scheduler.
...
// runtime metrics from managed control plane components (kube-apiserver, etcd, etc). See aka.ms/aks/controlplane-metrics
// for an overview.
type ManagedClusterAzureMonitorProfileMetricsControlPlane struct {
	// Enabled: Whether to enable or disable collection of control plane metrics by the Azure Managed Prometheus addon.
	// Defaults to disabled. See aka.ms/aks/controlplane-metrics for details.
	Enabled *bool `json:"enabled,omitempty"`
}
...
// Microsoft Defender settings for security gating. This validates container image eligibility for deployment based on
// Defender for Containers security findings. Using Admission Controller, it either audits or prevents deployment of images
// that do not meet security standards.
type ManagedClusterSecurityProfileDefenderSecurityGating struct {
	// AllowSecretAccess: In use only while registry access is granted by secret rather than managed identity. Sets whether to
	// grant the Defender gating agent access to cluster secrets for pulling images from registries. If secret access is denied
	// and the registry requires pull secrets, the add-on will not perform image validation. Default value is false.
	AllowSecretAccess *bool `json:"allowSecretAccess,omitempty"`

	// Enabled: Whether to enable Defender security gating. When enabled, the gating feature scans container images and audits
	// or blocks deployment of images that do not meet security standards according to configured security rules. For more
	// information, see https://aka.ms/KubernetesDefenderAuditRule.
	Enabled *bool `json:"enabled,omitempty"`

	// Identities: List of identities that the admission controller uses to pull security artifacts from registries. These are
	// the same identities used by the cluster to pull container images. For more information on configuring this identity, see
	// https://learn.microsoft.com/en-us/azure/defender-for-cloud/gated-deployment-infrastructure-as-code.
	Identities []ManagedClusterSecurityProfileDefenderSecurityGatingIdentity `json:"identities"`
}
...
type ManagedClusterWebAppRoutingGatewayAPIImplementations struct {
	// AppRoutingIstio: Configuration for using a sidecar-less Istio control plane for managed ingress via the Gateway API with
	// App Routing. See https://aka.ms/gateway-on-istio for information on using Istio for ingress via the Gateway API.
	AppRoutingIstio *ManagedClusterAppRoutingIstio `json:"appRoutingIstio,omitempty"`
}
...
// Configuration for using a sidecar-less Istio control plane for managed ingress via the Gateway API with App Routing. See
// https://aka.ms/gateway-on-istio for information on using Istio for ingress via the Gateway API.
type ManagedClusterAppRoutingIstio struct {
	// Mode: Whether to enable Istio as a Gateway API implementation for managed ingress with App Routing.
	Mode *GatewayAPIIstioEnabled `json:"mode,omitempty"`
}
...
const (
	GatewayAPIIstioEnabled_Disabled = GatewayAPIIstioEnabled("Disabled")
	GatewayAPIIstioEnabled_Enabled  = GatewayAPIIstioEnabled("Enabled")
)

// Mapping from string to GatewayAPIIstioEnabled
var gatewayAPIIstioEnabled_Values = map[string]GatewayAPIIstioEnabled{
	"disabled": GatewayAPIIstioEnabled_Disabled,
	"enabled":  GatewayAPIIstioEnabled_Enabled,
}
