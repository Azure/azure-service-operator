...
func (member *FleetsMember_STATUS) PopulateFromARM(owner genruntime.ArbitraryOwnerReference, armInput interface{}) error {
	...
	if typedInput.Properties != nil {
		if typedInput.Properties.Status != nil {
			var status1 FleetMemberStatus_STATUS
...
type FleetMemberStatus_STATUS struct {
	// LastOperationError: The last operation error of the fleet member
	LastOperationError *ErrorDetail_STATUS `json:"lastOperationError,omitempty"`

	// LastOperationId: The last operation ID for the fleet member
	LastOperationId *string `json:"lastOperationId,omitempty"`
}

var _ genruntime.FromARMConverter = &FleetMemberStatus_STATUS{}

// NewEmptyARMValue returns an empty ARM value suitable for deserializing into
func (status *FleetMemberStatus_STATUS) NewEmptyARMValue() genruntime.ARMResourceStatus {
	return &arm.FleetMemberStatus_STATUS{}
}

// PopulateFromARM populates a Kubernetes CRD object from an Azure ARM object
func (status *FleetMemberStatus_STATUS) PopulateFromARM(owner genruntime.ArbitraryOwnerReference, armInput interface{}) error {
	typedInput, ok := armInput.(arm.FleetMemberStatus_STATUS)
	if !ok {
		return fmt.Errorf("unexpected type supplied for PopulateFromARM() function. Expected arm.FleetMemberStatus_STATUS, got %T", armInput)
	}
