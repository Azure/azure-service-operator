...
import (
	"fmt"
	arm "github.com/Azure/azure-service-operator/v2/api/containerservice/v20260301/arm"
...
func (schedule *RelativeMonthlySchedule) AssignProperties_From_RelativeMonthlySchedule(source *storage.RelativeMonthlySchedule) error {
	...
	if source.WeekIndex != nil {
		...
		weekIndexTemp := genruntime.ToEnum(weekIndex, type_Values)
...
func (schedule *RelativeMonthlySchedule) Initialize_From_RelativeMonthlySchedule_STATUS(source *RelativeMonthlySchedule_STATUS) error {
	...
	// WeekIndex
	if source.WeekIndex != nil {
		weekIndex := genruntime.ToEnum(string(*source.WeekIndex), type_Values)
		schedule.WeekIndex = &weekIndex
	} else {
		schedule.WeekIndex = nil
	}
...
type RelativeMonthlySchedule_STATUS struct {
	...
	// WeekIndex: The week index. Specifies on which week of the month the dayOfWeek applies.
	WeekIndex *Type_STATUS `json:"weekIndex,omitempty"`
}
...
func (schedule *RelativeMonthlySchedule_STATUS) AssignProperties_From_RelativeMonthlySchedule_STATUS(source *storage.RelativeMonthlySchedule_STATUS) error {
	...
	if source.WeekIndex != nil {
		weekIndex := *source.WeekIndex
		weekIndexTemp := genruntime.ToEnum(weekIndex, type_STATUS_Values)
		schedule.WeekIndex = &weekIndexTemp
	} else {
		schedule.WeekIndex = nil
	}
...
// The week index. Specifies on which week of the month the dayOfWeek applies.
type Type_STATUS string

const (
	Type_STATUS_First  = Type_STATUS("First")
	Type_STATUS_Fourth = Type_STATUS("Fourth")
	Type_STATUS_Last   = Type_STATUS("Last")
	Type_STATUS_Second = Type_STATUS("Second")
	Type_STATUS_Third  = Type_STATUS("Third")
)

// Mapping from string to Type_STATUS
var type_STATUS_Values = map[string]Type_STATUS{
	"first":  Type_STATUS_First,
	"fourth": Type_STATUS_Fourth,
	"last":   Type_STATUS_Last,
	"second": Type_STATUS_Second,
	"third":  Type_STATUS_Third,
}
