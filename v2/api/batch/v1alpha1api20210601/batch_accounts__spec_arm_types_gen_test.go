...
// We first initialize autoStorageBasePropertiesARMGenerator with a simplified generator based on the
// fields with primitive types then replacing it with a more complex one that also handles complex fields
// to ensure any cycles in the object graph properly terminate.
func AutoStorageBasePropertiesARMGenerator() gopter.Gen {
	...
	generators = make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForAutoStorageBasePropertiesARM(generators)
	AddRelatedPropertyGeneratorsForAutoStorageBasePropertiesARM(generators)
	autoStorageBasePropertiesARMGenerator = gen.Struct(reflect.TypeOf(AutoStorageBasePropertiesARM{}), generators)
...
// AddRelatedPropertyGeneratorsForAutoStorageBasePropertiesARM is a factory method for creating gopter generators
func AddRelatedPropertyGeneratorsForAutoStorageBasePropertiesARM(gens map[string]gopter.Gen) {
	gens["NodeIdentityReference"] = gen.PtrOf(ComputeNodeIdentityReferenceARMGenerator())
}
...
func Test_ComputeNodeIdentityReferenceARM_WhenSerializedToJson_DeserializesAsEqual(t *testing.T) {
	t.Parallel()
	parameters := gopter.DefaultTestParameters()
	parameters.MaxSize = 10
	properties := gopter.NewProperties(parameters)
	properties.Property(
		"Round trip of ComputeNodeIdentityReferenceARM via JSON returns original",
		prop.ForAll(RunJSONSerializationTestForComputeNodeIdentityReferenceARM, ComputeNodeIdentityReferenceARMGenerator()))
	properties.TestingRun(t, gopter.NewFormatedReporter(true, 240, os.Stdout))
}

// RunJSONSerializationTestForComputeNodeIdentityReferenceARM runs a test to see if a specific instance of ComputeNodeIdentityReferenceARM round trips to JSON and back losslessly
func RunJSONSerializationTestForComputeNodeIdentityReferenceARM(subject ComputeNodeIdentityReferenceARM) string {
	// Serialize to JSON
	bin, err := json.Marshal(subject)
	if err != nil {
		return err.Error()
	...
	// Deserialize back into memory
	var actual ComputeNodeIdentityReferenceARM
	err = json.Unmarshal(bin, &actual)
	if err != nil {
		return err.Error()
	}
...
// Generator of ComputeNodeIdentityReferenceARM instances for property testing - lazily instantiated by
//ComputeNodeIdentityReferenceARMGenerator()
var computeNodeIdentityReferenceARMGenerator gopter.Gen

// ComputeNodeIdentityReferenceARMGenerator returns a generator of ComputeNodeIdentityReferenceARM instances for property testing.
func ComputeNodeIdentityReferenceARMGenerator() gopter.Gen {
	if computeNodeIdentityReferenceARMGenerator != nil {
		return computeNodeIdentityReferenceARMGenerator
	}

	generators := make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForComputeNodeIdentityReferenceARM(generators)
	computeNodeIdentityReferenceARMGenerator = gen.Struct(reflect.TypeOf(ComputeNodeIdentityReferenceARM{}), generators)

	return computeNodeIdentityReferenceARMGenerator
}

// AddIndependentPropertyGeneratorsForComputeNodeIdentityReferenceARM is a factory method for creating gopter generators
func AddIndependentPropertyGeneratorsForComputeNodeIdentityReferenceARM(gens map[string]gopter.Gen) {
