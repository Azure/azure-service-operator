...
import (
	"context"
	"fmt"
	arm "github.com/Azure/azure-service-operator/v2/api/dbformysql/v20241230/arm"
...
// +kubebuilder:validation:Enum={"2024-12-30"}
type APIVersion string

const APIVersion_Value = APIVersion("2024-12-30")
...
func (server *FlexibleServer_Spec) Initialize_From_FlexibleServer_STATUS(source *FlexibleServer_STATUS) error {
	...
	if source.MaintenancePolicy != nil {
		var maintenancePolicy MaintenancePolicy
		err := maintenancePolicy.Initialize_From_MaintenancePolicy_STATUS(source.MaintenancePolicy)
		if err != nil {
			return eris.Wrap(err, "calling Initialize_From_MaintenancePolicy_STATUS() to populate field MaintenancePolicy")
		}
		server.MaintenancePolicy = &maintenancePolicy
	} else {
		server.MaintenancePolicy = nil
...
func (availability *HighAvailability) Initialize_From_HighAvailability_STATUS(source *HighAvailability_STATUS) error {
	...
	// Mode
	if source.Mode != nil {
		mode := genruntime.ToEnum(string(*source.Mode), highAvailabilityMode_Values)
		availability.Mode = &mode
	} else {
		availability.Mode = nil
	}
...
func (properties *ImportSourceProperties) Initialize_From_ImportSourceProperties_STATUS(source *ImportSourceProperties_STATUS) error {
	...
	// StorageType
	if source.StorageType != nil {
		storageType := genruntime.ToEnum(string(*source.StorageType), importSourceStorageType_Values)
		properties.StorageType = &storageType
	} else {
		properties.StorageType = nil
	}
...
// Initialize_From_MaintenancePolicy_STATUS populates our MaintenancePolicy from the provided source MaintenancePolicy_STATUS
func (policy *MaintenancePolicy) Initialize_From_MaintenancePolicy_STATUS(source *MaintenancePolicy_STATUS) error {

	// PatchStrategy
	if source.PatchStrategy != nil {
		patchStrategy := genruntime.ToEnum(string(*source.PatchStrategy), patchStrategy_Values)
		policy.PatchStrategy = &patchStrategy
	} else {
		policy.PatchStrategy = nil
	}
...
func (window *MaintenanceWindow) Initialize_From_MaintenanceWindow_STATUS(source *MaintenanceWindow_STATUS) error {
	...
	// BatchOfMaintenance
	if source.BatchOfMaintenance != nil {
		batchOfMaintenance := genruntime.ToEnum(string(*source.BatchOfMaintenance), batchOfMaintenance_Values)
		window.BatchOfMaintenance = &batchOfMaintenance
	} else {
		window.BatchOfMaintenance = nil
	}
...
func (serverSku *MySQLServerSku) Initialize_From_MySQLServerSku_STATUS(source *MySQLServerSku_STATUS) error {
	...
	// Tier
	if source.Tier != nil {
		tier := genruntime.ToEnum(string(*source.Tier), serverSkuTier_Values)
		serverSku.Tier = &tier
	} else {
		serverSku.Tier = nil
	}
...
func (storage *Storage) Initialize_From_Storage_STATUS(source *Storage_STATUS) error {
	...
	// AutoIoScaling
	if source.AutoIoScaling != nil {
		autoIoScaling := genruntime.ToEnum(string(*source.AutoIoScaling), storage_AutoIoScaling_Values)
		storage.AutoIoScaling = &autoIoScaling
	} else {
		storage.AutoIoScaling = nil
	}
	...
	// LogOnDisk
	if source.LogOnDisk != nil {
		logOnDisk := genruntime.ToEnum(string(*source.LogOnDisk), storage_LogOnDisk_Values)
		storage.LogOnDisk = &logOnDisk
	} else {
		storage.LogOnDisk = nil
	}
	...
	// StorageRedundancy
	if source.StorageRedundancy != nil {
		storageRedundancy := genruntime.ToEnum(string(*source.StorageRedundancy), storage_StorageRedundancy_Values)
		storage.StorageRedundancy = &storageRedundancy
	} else {
		storage.StorageRedundancy = nil
	}
