...
func (record *PrivateDnsZonesTXTRecord) InitializeSpec(status genruntime.ConvertibleStatus) error {
	if s, ok := status.(*PrivateDnsZonesTXTRecord_STATUS); ok {
		return record.Spec.Initialize_From_PrivateDnsZonesTXTRecord_STATUS(s)
	}

	return fmt.Errorf("expected Status of type PrivateDnsZonesTXTRecord_STATUS but received %T instead", status)
}
...
func (record *PrivateDnsZonesTXTRecord) validateConfigMapDestinations() (admission.Warnings, error) {
	if record.Spec.OperatorSpec == nil {
		return nil, nil
	}
...
func (record *PrivateDnsZonesTXTRecord) validateSecretDestinations() (admission.Warnings, error) {
	if record.Spec.OperatorSpec == nil {
		return nil, nil
	}
