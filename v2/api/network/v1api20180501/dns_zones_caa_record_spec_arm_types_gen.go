// Code generated by azure-service-operator-codegen. DO NOT EDIT.
// Copyright (c) Microsoft Corporation.
// Licensed under the MIT license.
package v1api20180501

import "github.com/Azure/azure-service-operator/v2/pkg/genruntime"

type DnsZonesCAARecord_Spec_ARM struct {
	Name string `json:"name,omitempty"`

	// Properties: The properties of the record set.
	Properties *RecordSetProperties_ARM `json:"properties,omitempty"`
}

var _ genruntime.ARMResourceSpec = &DnsZonesCAARecord_Spec_ARM{}

// GetAPIVersion returns the ARM API version of the resource. This is always "2018-05-01"
func (record DnsZonesCAARecord_Spec_ARM) GetAPIVersion() string {
	return "2018-05-01"
}

// GetName returns the Name of the resource
func (record *DnsZonesCAARecord_Spec_ARM) GetName() string {
	return record.Name
}

// GetType returns the ARM Type of the resource. This is always "Microsoft.Network/dnsZones/CAA"
func (record *DnsZonesCAARecord_Spec_ARM) GetType() string {
	return "Microsoft.Network/dnsZones/CAA"
}
