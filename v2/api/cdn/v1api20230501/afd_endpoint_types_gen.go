...
func (endpoint *AfdEndpoint_Spec) PopulateFromARM(owner genruntime.ArbitraryOwnerReference, armInput interface{}) error {
	typedInput, ok := armInput.(AfdEndpoint_Spec_ARM)
	if !ok {
		return fmt.Errorf("unexpected type supplied for PopulateFromARM() function. Expected AfdEndpoint_Spec_ARM, got %T", armInput)
	}
...
func (endpoint *AfdEndpoint_STATUS) NewEmptyARMValue() genruntime.ARMResourceStatus {
	return &AfdEndpoint_STATUS_ARM{}
}

// PopulateFromARM populates a Kubernetes CRD object from an Azure ARM object
func (endpoint *AfdEndpoint_STATUS) PopulateFromARM(owner genruntime.ArbitraryOwnerReference, armInput interface{}) error {
	typedInput, ok := armInput.(AfdEndpoint_STATUS_ARM)
	if !ok {
		return fmt.Errorf("unexpected type supplied for PopulateFromARM() function. Expected AfdEndpoint_STATUS_ARM, got %T", armInput)
	}
