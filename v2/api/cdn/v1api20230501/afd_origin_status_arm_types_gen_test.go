...
func Test_AfdOrigin_STATUS_ARM_WhenSerializedToJson_DeserializesAsEqual(t *testing.T) {
	t.Parallel()
	parameters := gopter.DefaultTestParameters()
	parameters.MinSuccessfulTests = 80
	parameters.MaxSize = 3
	properties := gopter.NewProperties(parameters)
	properties.Property(
		"Round trip of AfdOrigin_STATUS_ARM via JSON returns original",
		prop.ForAll(RunJSONSerializationTestForAfdOrigin_STATUS_ARM, AfdOrigin_STATUS_ARMGenerator()))
	properties.TestingRun(t, gopter.NewFormatedReporter(true, 240, os.Stdout))
}

// RunJSONSerializationTestForAfdOrigin_STATUS_ARM runs a test to see if a specific instance of AfdOrigin_STATUS_ARM round trips to JSON and back losslessly
func RunJSONSerializationTestForAfdOrigin_STATUS_ARM(subject AfdOrigin_STATUS_ARM) string {
	// Serialize to JSON
	bin, err := json.Marshal(subject)
	if err != nil {
		return err.Error()
	...
	// Deserialize back into memory
	var actual AfdOrigin_STATUS_ARM
	err = json.Unmarshal(bin, &actual)
	if err != nil {
		return err.Error()
	}
...
// Generator of AfdOrigin_STATUS_ARM instances for property testing - lazily instantiated by
// AfdOrigin_STATUS_ARMGenerator()
var afdOrigin_STATUS_ARMGenerator gopter.Gen

// AfdOrigin_STATUS_ARMGenerator returns a generator of AfdOrigin_STATUS_ARM instances for property testing.
// We first initialize afdOrigin_STATUS_ARMGenerator with a simplified generator based on the
// fields with primitive types then replacing it with a more complex one that also handles complex fields
// to ensure any cycles in the object graph properly terminate.
func AfdOrigin_STATUS_ARMGenerator() gopter.Gen {
	if afdOrigin_STATUS_ARMGenerator != nil {
		return afdOrigin_STATUS_ARMGenerator
	}

	generators := make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForAfdOrigin_STATUS_ARM(generators)
	afdOrigin_STATUS_ARMGenerator = gen.Struct(reflect.TypeOf(AfdOrigin_STATUS_ARM{}), generators)

	// The above call to gen.Struct() captures the map, so create a new one
	generators = make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForAfdOrigin_STATUS_ARM(generators)
	AddRelatedPropertyGeneratorsForAfdOrigin_STATUS_ARM(generators)
	afdOrigin_STATUS_ARMGenerator = gen.Struct(reflect.TypeOf(AfdOrigin_STATUS_ARM{}), generators)

	return afdOrigin_STATUS_ARMGenerator
}

// AddIndependentPropertyGeneratorsForAfdOrigin_STATUS_ARM is a factory method for creating gopter generators
func AddIndependentPropertyGeneratorsForAfdOrigin_STATUS_ARM(gens map[string]gopter.Gen) {
	gens["Id"] = gen.PtrOf(gen.AlphaString())
	gens["Name"] = gen.PtrOf(gen.AlphaString())
	gens["Type"] = gen.PtrOf(gen.AlphaString())
}

// AddRelatedPropertyGeneratorsForAfdOrigin_STATUS_ARM is a factory method for creating gopter generators
func AddRelatedPropertyGeneratorsForAfdOrigin_STATUS_ARM(gens map[string]gopter.Gen) {
