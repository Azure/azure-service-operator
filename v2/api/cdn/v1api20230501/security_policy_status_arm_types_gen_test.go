...
func Test_SecurityPolicy_STATUS_ARM_WhenSerializedToJson_DeserializesAsEqual(t *testing.T) {
	t.Parallel()
	parameters := gopter.DefaultTestParameters()
	parameters.MinSuccessfulTests = 80
	parameters.MaxSize = 3
	properties := gopter.NewProperties(parameters)
	properties.Property(
		"Round trip of SecurityPolicy_STATUS_ARM via JSON returns original",
		prop.ForAll(RunJSONSerializationTestForSecurityPolicy_STATUS_ARM, SecurityPolicy_STATUS_ARMGenerator()))
	properties.TestingRun(t, gopter.NewFormatedReporter(true, 240, os.Stdout))
}

// RunJSONSerializationTestForSecurityPolicy_STATUS_ARM runs a test to see if a specific instance of SecurityPolicy_STATUS_ARM round trips to JSON and back losslessly
func RunJSONSerializationTestForSecurityPolicy_STATUS_ARM(subject SecurityPolicy_STATUS_ARM) string {
	// Serialize to JSON
	bin, err := json.Marshal(subject)
	if err != nil {
		return err.Error()
	...
	// Deserialize back into memory
	var actual SecurityPolicy_STATUS_ARM
	err = json.Unmarshal(bin, &actual)
	if err != nil {
		return err.Error()
	}
...
// Generator of SecurityPolicy_STATUS_ARM instances for property testing - lazily instantiated by
// SecurityPolicy_STATUS_ARMGenerator()
var securityPolicy_STATUS_ARMGenerator gopter.Gen

// SecurityPolicy_STATUS_ARMGenerator returns a generator of SecurityPolicy_STATUS_ARM instances for property testing.
// We first initialize securityPolicy_STATUS_ARMGenerator with a simplified generator based on the
// fields with primitive types then replacing it with a more complex one that also handles complex fields
// to ensure any cycles in the object graph properly terminate.
func SecurityPolicy_STATUS_ARMGenerator() gopter.Gen {
	if securityPolicy_STATUS_ARMGenerator != nil {
		return securityPolicy_STATUS_ARMGenerator
	}

	generators := make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForSecurityPolicy_STATUS_ARM(generators)
	securityPolicy_STATUS_ARMGenerator = gen.Struct(reflect.TypeOf(SecurityPolicy_STATUS_ARM{}), generators)

	// The above call to gen.Struct() captures the map, so create a new one
	generators = make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForSecurityPolicy_STATUS_ARM(generators)
	AddRelatedPropertyGeneratorsForSecurityPolicy_STATUS_ARM(generators)
	securityPolicy_STATUS_ARMGenerator = gen.Struct(reflect.TypeOf(SecurityPolicy_STATUS_ARM{}), generators)

	return securityPolicy_STATUS_ARMGenerator
}

// AddIndependentPropertyGeneratorsForSecurityPolicy_STATUS_ARM is a factory method for creating gopter generators
func AddIndependentPropertyGeneratorsForSecurityPolicy_STATUS_ARM(gens map[string]gopter.Gen) {
	gens["Id"] = gen.PtrOf(gen.AlphaString())
	gens["Name"] = gen.PtrOf(gen.AlphaString())
	gens["Type"] = gen.PtrOf(gen.AlphaString())
}

// AddRelatedPropertyGeneratorsForSecurityPolicy_STATUS_ARM is a factory method for creating gopter generators
func AddRelatedPropertyGeneratorsForSecurityPolicy_STATUS_ARM(gens map[string]gopter.Gen) {
