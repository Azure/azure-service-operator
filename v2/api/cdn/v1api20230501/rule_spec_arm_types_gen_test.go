...
func Test_Rule_Spec_ARM_WhenSerializedToJson_DeserializesAsEqual(t *testing.T) {
	t.Parallel()
	parameters := gopter.DefaultTestParameters()
	...
	properties := gopter.NewProperties(parameters)
	properties.Property(
		"Round trip of Rule_Spec_ARM via JSON returns original",
		prop.ForAll(RunJSONSerializationTestForRule_Spec_ARM, Rule_Spec_ARMGenerator()))
	properties.TestingRun(t, gopter.NewFormatedReporter(true, 240, os.Stdout))
}

// RunJSONSerializationTestForRule_Spec_ARM runs a test to see if a specific instance of Rule_Spec_ARM round trips to JSON and back losslessly
func RunJSONSerializationTestForRule_Spec_ARM(subject Rule_Spec_ARM) string {
	// Serialize to JSON
	bin, err := json.Marshal(subject)
	if err != nil {
		return err.Error()
...
// Generator of Rule_Spec_ARM instances for property testing - lazily instantiated by Rule_Spec_ARMGenerator()
var rule_Spec_ARMGenerator gopter.Gen

// Rule_Spec_ARMGenerator returns a generator of Rule_Spec_ARM instances for property testing.
// We first initialize rule_Spec_ARMGenerator with a simplified generator based on the
// fields with primitive types then replacing it with a more complex one that also handles complex fields
// to ensure any cycles in the object graph properly terminate.
func Rule_Spec_ARMGenerator() gopter.Gen {
	if rule_Spec_ARMGenerator != nil {
		return rule_Spec_ARMGenerator
	}

	generators := make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForRule_Spec_ARM(generators)
	rule_Spec_ARMGenerator = gen.Struct(reflect.TypeOf(Rule_Spec_ARM{}), generators)

	// The above call to gen.Struct() captures the map, so create a new one
	generators = make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForRule_Spec_ARM(generators)
	AddRelatedPropertyGeneratorsForRule_Spec_ARM(generators)
	rule_Spec_ARMGenerator = gen.Struct(reflect.TypeOf(Rule_Spec_ARM{}), generators)

	return rule_Spec_ARMGenerator
}

// AddIndependentPropertyGeneratorsForRule_Spec_ARM is a factory method for creating gopter generators
func AddIndependentPropertyGeneratorsForRule_Spec_ARM(gens map[string]gopter.Gen) {
	gens["Name"] = gen.AlphaString()
}

// AddRelatedPropertyGeneratorsForRule_Spec_ARM is a factory method for creating gopter generators
func AddRelatedPropertyGeneratorsForRule_Spec_ARM(gens map[string]gopter.Gen) {
