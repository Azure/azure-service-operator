...
func Test_Route_Spec_ARM_WhenSerializedToJson_DeserializesAsEqual(t *testing.T) {
	t.Parallel()
	parameters := gopter.DefaultTestParameters()
	...
	properties := gopter.NewProperties(parameters)
	properties.Property(
		"Round trip of Route_Spec_ARM via JSON returns original",
		prop.ForAll(RunJSONSerializationTestForRoute_Spec_ARM, Route_Spec_ARMGenerator()))
	properties.TestingRun(t, gopter.NewFormatedReporter(true, 240, os.Stdout))
}

// RunJSONSerializationTestForRoute_Spec_ARM runs a test to see if a specific instance of Route_Spec_ARM round trips to JSON and back losslessly
func RunJSONSerializationTestForRoute_Spec_ARM(subject Route_Spec_ARM) string {
	// Serialize to JSON
	bin, err := json.Marshal(subject)
	if err != nil {
		return err.Error()
...
// Generator of Route_Spec_ARM instances for property testing - lazily instantiated by Route_Spec_ARMGenerator()
var route_Spec_ARMGenerator gopter.Gen

// Route_Spec_ARMGenerator returns a generator of Route_Spec_ARM instances for property testing.
// We first initialize route_Spec_ARMGenerator with a simplified generator based on the
// fields with primitive types then replacing it with a more complex one that also handles complex fields
// to ensure any cycles in the object graph properly terminate.
func Route_Spec_ARMGenerator() gopter.Gen {
	if route_Spec_ARMGenerator != nil {
		return route_Spec_ARMGenerator
	}

	generators := make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForRoute_Spec_ARM(generators)
	route_Spec_ARMGenerator = gen.Struct(reflect.TypeOf(Route_Spec_ARM{}), generators)

	// The above call to gen.Struct() captures the map, so create a new one
	generators = make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForRoute_Spec_ARM(generators)
	AddRelatedPropertyGeneratorsForRoute_Spec_ARM(generators)
	route_Spec_ARMGenerator = gen.Struct(reflect.TypeOf(Route_Spec_ARM{}), generators)

	return route_Spec_ARMGenerator
}

// AddIndependentPropertyGeneratorsForRoute_Spec_ARM is a factory method for creating gopter generators
func AddIndependentPropertyGeneratorsForRoute_Spec_ARM(gens map[string]gopter.Gen) {
	gens["Name"] = gen.AlphaString()
}

// AddRelatedPropertyGeneratorsForRoute_Spec_ARM is a factory method for creating gopter generators
func AddRelatedPropertyGeneratorsForRoute_Spec_ARM(gens map[string]gopter.Gen) {
