...
func Test_SiteSourceControlProperties_WhenSerializedToJson_DeserializesAsEqual(t *testing.T) {
	t.Parallel()
	parameters := gopter.DefaultTestParameters()
	parameters.MinSuccessfulTests = 100
	parameters.MaxSize = 3
	properties := gopter.NewProperties(parameters)
	properties.Property(
		"Round trip of SiteSourceControlProperties via JSON returns original",
		prop.ForAll(RunJSONSerializationTestForSiteSourceControlProperties, SiteSourceControlPropertiesGenerator()))
	properties.TestingRun(t, gopter.NewFormatedReporter(true, 240, os.Stdout))
}

// RunJSONSerializationTestForSiteSourceControlProperties runs a test to see if a specific instance of SiteSourceControlProperties round trips to JSON and back losslessly
func RunJSONSerializationTestForSiteSourceControlProperties(subject SiteSourceControlProperties) string {
	// Serialize to JSON
	bin, err := json.Marshal(subject)
	if err != nil {
		return err.Error()
	...
	// Deserialize back into memory
	var actual SiteSourceControlProperties
	err = json.Unmarshal(bin, &actual)
	if err != nil {
		return err.Error()
	}
...
// Generator of SiteSourceControlProperties instances for property testing - lazily instantiated by
// SiteSourceControlPropertiesGenerator()
var siteSourceControlPropertiesGenerator gopter.Gen

// SiteSourceControlPropertiesGenerator returns a generator of SiteSourceControlProperties instances for property testing.
// We first initialize siteSourceControlPropertiesGenerator with a simplified generator based on the
// fields with primitive types then replacing it with a more complex one that also handles complex fields
// to ensure any cycles in the object graph properly terminate.
func SiteSourceControlPropertiesGenerator() gopter.Gen {
	if siteSourceControlPropertiesGenerator != nil {
		return siteSourceControlPropertiesGenerator
	}

	generators := make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForSiteSourceControlProperties(generators)
	siteSourceControlPropertiesGenerator = gen.Struct(reflect.TypeOf(SiteSourceControlProperties{}), generators)

	// The above call to gen.Struct() captures the map, so create a new one
	generators = make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForSiteSourceControlProperties(generators)
	AddRelatedPropertyGeneratorsForSiteSourceControlProperties(generators)
	siteSourceControlPropertiesGenerator = gen.Struct(reflect.TypeOf(SiteSourceControlProperties{}), generators)

	return siteSourceControlPropertiesGenerator
}

// AddIndependentPropertyGeneratorsForSiteSourceControlProperties is a factory method for creating gopter generators
func AddIndependentPropertyGeneratorsForSiteSourceControlProperties(gens map[string]gopter.Gen) {
...
// AddRelatedPropertyGeneratorsForSiteSourceControlProperties is a factory method for creating gopter generators
func AddRelatedPropertyGeneratorsForSiteSourceControlProperties(gens map[string]gopter.Gen) {
...
func AddRelatedPropertyGeneratorsForSitesSourcecontrol_Spec(gens map[string]gopter.Gen) {
	gens["Properties"] = gen.PtrOf(SiteSourceControlPropertiesGenerator())
}
