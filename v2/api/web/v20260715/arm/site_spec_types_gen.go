...
type SiteProperties struct {
	// AiIntegration: AI integration configuration for the app.
	AiIntegration *AiIntegration `json:"aiIntegration,omitempty"`
	...
	// MaintenanceEnabled: <code>true</code> if the app is in maintenance mode; otherwise, <code>false</code>. Setting this
	// value to true puts the app in maintenance mode, making it return the maintenance custom error page. The maintenance
	// custom error page is required to set the site to maintenance. Default value is <code>false</code>.
	...
	// PlatformReleaseChannel: The platform release channel for the site. Latest receives updates earliest, followed by
	// Standard, then Extended.
	PlatformReleaseChannel *PlatformReleaseChannel `json:"platformReleaseChannel,omitempty"`
	...
	// SiteScopedCertificatesEnabled: <code>true</code> if site scoped certificates are enabled; otherwise, <code>false</code>.
	SiteScopedCertificatesEnabled *bool `json:"siteScopedCertificatesEnabled,omitempty"`
...
type AiIntegration struct {
	// ApiSpecPath: Relative path to the API specification file from /home in site files.
	ApiSpecPath *string `json:"apiSpecPath,omitempty"`

	// Mcp: MCP configuration.
	Mcp *McpSettings `json:"mcp,omitempty"`

	// SiteAuth: External auth configuration for the MCP endpoint (used when EasyAuth is not enabled).
...
type OutboundVnetRouting struct {
	...
	// ManagedIdentityTraffic: Enables Managed Identity operations over virtual network.
	ManagedIdentityTraffic *bool `json:"managedIdentityTraffic,omitempty"`
}

// The platform release channel for the site.
// +kubebuilder:validation:Enum={"Extended","Latest","Standard"}
type PlatformReleaseChannel string

const (
	PlatformReleaseChannel_Extended = PlatformReleaseChannel("Extended")
	PlatformReleaseChannel_Latest   = PlatformReleaseChannel("Latest")
	PlatformReleaseChannel_Standard = PlatformReleaseChannel("Standard")
)

// Mapping from string to PlatformReleaseChannel
var platformReleaseChannel_Values = map[string]PlatformReleaseChannel{
	"extended": PlatformReleaseChannel_Extended,
	"latest":   PlatformReleaseChannel_Latest,
	"standard": PlatformReleaseChannel_Standard,
}
...
type McpSettings struct {
	// Servers: Array of MCP server configurations. This array is replaced in its entirety on update; individual servers cannot
	// be patched independently. An empty array signals that MCP should be disabled/deleted.
...
// At minimum, `scopes` and either
// `wellKnownOpenIdConfiguration` or `issuer` must be provided.
type SiteAuthInfo struct {
	// Audience: Expected 'aud' claim.
	Audience *string `json:"audience,omitempty"`

	// Issuer: Expected 'iss' claim. Required for custom site authentication if `wellKnownOpenIdConfiguration` is not provided.
	Issuer *string `json:"issuer,omitempty"`

	// JwksUri: JWKS URL for verifying JWT signatures.
	JwksUri *string `json:"jwksUri,omitempty"`

	// Scopes: OAuth scopes for Protected Resource Metadata (PRM). Required for custom site authentication.
	Scopes []string `json:"scopes,omitempty"`

	// WellKnownOpenIdConfiguration: OIDC metadata URL. Required for custom site authentication if `issuer` is not provided.
...
type McpServerConfig struct {
	...
	// Enabled: Whether the MCP server is enabled. Defaults to true when not specified on creation.
	Enabled *bool `json:"enabled,omitempty"`

	// Endpoint: The endpoint path for the MCP server.
	Endpoint *string `json:"endpoint,omitempty"`

	// Name: The name of the MCP server.
	Name *string `json:"name,omitempty"`

	// ToolList: The list of tools exposed by this MCP server.
...
// +kubebuilder:validation:Enum={"custom","dotnet-isolated","go","java","node","powershell","python"}
type RuntimeName string

const (
	RuntimeName_Custom         = RuntimeName("custom")
	RuntimeName_DotnetIsolated = RuntimeName("dotnet-isolated")
	RuntimeName_Go             = RuntimeName("go")
