...
func AddRelatedPropertyGeneratorsForApiManagementServiceProperties_STATUS_ARM(gens map[string]gopter.Gen) {
	...
	gens["ConfigurationApi"] = gen.PtrOf(ConfigurationApi_STATUS_ARMGenerator())
...
func Test_ConfigurationApi_STATUS_ARM_WhenSerializedToJson_DeserializesAsEqual(t *testing.T) {
	t.Parallel()
	parameters := gopter.DefaultTestParameters()
	parameters.MinSuccessfulTests = 80
	parameters.MaxSize = 3
	properties := gopter.NewProperties(parameters)
	properties.Property(
		"Round trip of ConfigurationApi_STATUS_ARM via JSON returns original",
		prop.ForAll(RunJSONSerializationTestForConfigurationApi_STATUS_ARM, ConfigurationApi_STATUS_ARMGenerator()))
	properties.TestingRun(t, gopter.NewFormatedReporter(true, 240, os.Stdout))
}

// RunJSONSerializationTestForConfigurationApi_STATUS_ARM runs a test to see if a specific instance of ConfigurationApi_STATUS_ARM round trips to JSON and back losslessly
func RunJSONSerializationTestForConfigurationApi_STATUS_ARM(subject ConfigurationApi_STATUS_ARM) string {
	// Serialize to JSON
	bin, err := json.Marshal(subject)
	if err != nil {
		return err.Error()
	...
	// Deserialize back into memory
	var actual ConfigurationApi_STATUS_ARM
	err = json.Unmarshal(bin, &actual)
	if err != nil {
		return err.Error()
	}
...
// Generator of ConfigurationApi_STATUS_ARM instances for property testing - lazily instantiated by
// ConfigurationApi_STATUS_ARMGenerator()
var configurationApi_STATUS_ARMGenerator gopter.Gen

// ConfigurationApi_STATUS_ARMGenerator returns a generator of ConfigurationApi_STATUS_ARM instances for property testing.
func ConfigurationApi_STATUS_ARMGenerator() gopter.Gen {
	if configurationApi_STATUS_ARMGenerator != nil {
		return configurationApi_STATUS_ARMGenerator
	}

	generators := make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForConfigurationApi_STATUS_ARM(generators)
	configurationApi_STATUS_ARMGenerator = gen.Struct(reflect.TypeOf(ConfigurationApi_STATUS_ARM{}), generators)

	return configurationApi_STATUS_ARMGenerator
}

// AddIndependentPropertyGeneratorsForConfigurationApi_STATUS_ARM is a factory method for creating gopter generators
func AddIndependentPropertyGeneratorsForConfigurationApi_STATUS_ARM(gens map[string]gopter.Gen) {
