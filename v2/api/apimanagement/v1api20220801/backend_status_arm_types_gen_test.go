...
func Test_Backend_STATUS_ARM_WhenSerializedToJson_DeserializesAsEqual(t *testing.T) {
	t.Parallel()
	parameters := gopter.DefaultTestParameters()
	parameters.MinSuccessfulTests = 80
	parameters.MaxSize = 3
	properties := gopter.NewProperties(parameters)
	properties.Property(
		"Round trip of Backend_STATUS_ARM via JSON returns original",
		prop.ForAll(RunJSONSerializationTestForBackend_STATUS_ARM, Backend_STATUS_ARMGenerator()))
	properties.TestingRun(t, gopter.NewFormatedReporter(true, 240, os.Stdout))
}

// RunJSONSerializationTestForBackend_STATUS_ARM runs a test to see if a specific instance of Backend_STATUS_ARM round trips to JSON and back losslessly
func RunJSONSerializationTestForBackend_STATUS_ARM(subject Backend_STATUS_ARM) string {
	// Serialize to JSON
	bin, err := json.Marshal(subject)
	if err != nil {
		return err.Error()
...
// Generator of Backend_STATUS_ARM instances for property testing - lazily instantiated by Backend_STATUS_ARMGenerator()
var backend_STATUS_ARMGenerator gopter.Gen

// Backend_STATUS_ARMGenerator returns a generator of Backend_STATUS_ARM instances for property testing.
// We first initialize backend_STATUS_ARMGenerator with a simplified generator based on the
// fields with primitive types then replacing it with a more complex one that also handles complex fields
// to ensure any cycles in the object graph properly terminate.
func Backend_STATUS_ARMGenerator() gopter.Gen {
	if backend_STATUS_ARMGenerator != nil {
		return backend_STATUS_ARMGenerator
	}

	generators := make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForBackend_STATUS_ARM(generators)
	backend_STATUS_ARMGenerator = gen.Struct(reflect.TypeOf(Backend_STATUS_ARM{}), generators)

	// The above call to gen.Struct() captures the map, so create a new one
	generators = make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForBackend_STATUS_ARM(generators)
	AddRelatedPropertyGeneratorsForBackend_STATUS_ARM(generators)
	backend_STATUS_ARMGenerator = gen.Struct(reflect.TypeOf(Backend_STATUS_ARM{}), generators)

	return backend_STATUS_ARMGenerator
}

// AddIndependentPropertyGeneratorsForBackend_STATUS_ARM is a factory method for creating gopter generators
func AddIndependentPropertyGeneratorsForBackend_STATUS_ARM(gens map[string]gopter.Gen) {
	gens["Id"] = gen.PtrOf(gen.AlphaString())
	gens["Name"] = gen.PtrOf(gen.AlphaString())
	gens["Type"] = gen.PtrOf(gen.AlphaString())
}

// AddRelatedPropertyGeneratorsForBackend_STATUS_ARM is a factory method for creating gopter generators
func AddRelatedPropertyGeneratorsForBackend_STATUS_ARM(gens map[string]gopter.Gen) {
