// Code generated by azure-service-operator-codegen. DO NOT EDIT.
// Copyright (c) Microsoft Corporation.
// Licensed under the MIT license.
package v1api20220801

import "github.com/Azure/azure-service-operator/v2/pkg/genruntime"

type AuthorizationProvider_Spec_ARM struct {
	Name string `json:"name,omitempty"`

	// Properties: Properties of the Authorization Provider Contract.
	Properties *AuthorizationProviderContractProperties_ARM `json:"properties,omitempty"`
}

var _ genruntime.ARMResourceSpec = &AuthorizationProvider_Spec_ARM{}

// GetAPIVersion returns the ARM API version of the resource. This is always "2022-08-01"
func (provider AuthorizationProvider_Spec_ARM) GetAPIVersion() string {
	return "2022-08-01"
}

// GetName returns the Name of the resource
func (provider *AuthorizationProvider_Spec_ARM) GetName() string {
	return provider.Name
}

// GetType returns the ARM Type of the resource. This is always "Microsoft.ApiManagement/service/authorizationProviders"
func (provider *AuthorizationProvider_Spec_ARM) GetType() string {
	return "Microsoft.ApiManagement/service/authorizationProviders"
}

// Authorization Provider details.
type AuthorizationProviderContractProperties_ARM struct {
	// DisplayName: Authorization Provider name. Must be 1 to 300 characters long.
	DisplayName *string `json:"displayName,omitempty"`

	// IdentityProvider: Identity provider name. Must be 1 to 300 characters long.
	IdentityProvider *string `json:"identityProvider,omitempty"`

	// Oauth2: OAuth2 settings
	Oauth2 *AuthorizationProviderOAuth2Settings_ARM `json:"oauth2,omitempty"`
}

// OAuth2 settings details
type AuthorizationProviderOAuth2Settings_ARM struct {
	// GrantTypes: OAuth2 settings
	GrantTypes *AuthorizationProviderOAuth2GrantTypes_ARM `json:"grantTypes,omitempty"`

	// RedirectUrl: Redirect URL to be set in the OAuth application.
	RedirectUrl *string `json:"redirectUrl,omitempty"`
}

// Authorization Provider oauth2 grant types settings
type AuthorizationProviderOAuth2GrantTypes_ARM struct {
	// AuthorizationCode: OAuth2 authorization code grant parameters
	AuthorizationCode map[string]string `json:"authorizationCode,omitempty"`

	// ClientCredentials: OAuth2 client credential grant parameters
	ClientCredentials map[string]string `json:"clientCredentials,omitempty"`
}
