...
func Test_Api_Spec_ARM_WhenSerializedToJson_DeserializesAsEqual(t *testing.T) {
	t.Parallel()
	parameters := gopter.DefaultTestParameters()
	...
	properties := gopter.NewProperties(parameters)
	properties.Property(
		"Round trip of Api_Spec_ARM via JSON returns original",
		prop.ForAll(RunJSONSerializationTestForApi_Spec_ARM, Api_Spec_ARMGenerator()))
	properties.TestingRun(t, gopter.NewFormatedReporter(true, 240, os.Stdout))
}

// RunJSONSerializationTestForApi_Spec_ARM runs a test to see if a specific instance of Api_Spec_ARM round trips to JSON and back losslessly
func RunJSONSerializationTestForApi_Spec_ARM(subject Api_Spec_ARM) string {
	// Serialize to JSON
	bin, err := json.Marshal(subject)
	if err != nil {
		return err.Error()
...
// Generator of Api_Spec_ARM instances for property testing - lazily instantiated by Api_Spec_ARMGenerator()
var api_Spec_ARMGenerator gopter.Gen

// Api_Spec_ARMGenerator returns a generator of Api_Spec_ARM instances for property testing.
// We first initialize api_Spec_ARMGenerator with a simplified generator based on the
// fields with primitive types then replacing it with a more complex one that also handles complex fields
// to ensure any cycles in the object graph properly terminate.
func Api_Spec_ARMGenerator() gopter.Gen {
	if api_Spec_ARMGenerator != nil {
		return api_Spec_ARMGenerator
	}

	generators := make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForApi_Spec_ARM(generators)
	api_Spec_ARMGenerator = gen.Struct(reflect.TypeOf(Api_Spec_ARM{}), generators)

	// The above call to gen.Struct() captures the map, so create a new one
	generators = make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForApi_Spec_ARM(generators)
	AddRelatedPropertyGeneratorsForApi_Spec_ARM(generators)
	api_Spec_ARMGenerator = gen.Struct(reflect.TypeOf(Api_Spec_ARM{}), generators)

	return api_Spec_ARMGenerator
}

// AddIndependentPropertyGeneratorsForApi_Spec_ARM is a factory method for creating gopter generators
func AddIndependentPropertyGeneratorsForApi_Spec_ARM(gens map[string]gopter.Gen) {
	gens["Name"] = gen.AlphaString()
}

// AddRelatedPropertyGeneratorsForApi_Spec_ARM is a factory method for creating gopter generators
func AddRelatedPropertyGeneratorsForApi_Spec_ARM(gens map[string]gopter.Gen) {
