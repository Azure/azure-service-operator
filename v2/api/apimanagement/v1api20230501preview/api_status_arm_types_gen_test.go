...
func Test_Api_STATUS_ARM_WhenSerializedToJson_DeserializesAsEqual(t *testing.T) {
	t.Parallel()
	parameters := gopter.DefaultTestParameters()
	parameters.MinSuccessfulTests = 80
	parameters.MaxSize = 3
	properties := gopter.NewProperties(parameters)
	properties.Property(
		"Round trip of Api_STATUS_ARM via JSON returns original",
		prop.ForAll(RunJSONSerializationTestForApi_STATUS_ARM, Api_STATUS_ARMGenerator()))
	properties.TestingRun(t, gopter.NewFormatedReporter(true, 240, os.Stdout))
}

// RunJSONSerializationTestForApi_STATUS_ARM runs a test to see if a specific instance of Api_STATUS_ARM round trips to JSON and back losslessly
func RunJSONSerializationTestForApi_STATUS_ARM(subject Api_STATUS_ARM) string {
	// Serialize to JSON
	bin, err := json.Marshal(subject)
	if err != nil {
		return err.Error()
...
// Generator of Api_STATUS_ARM instances for property testing - lazily instantiated by Api_STATUS_ARMGenerator()
var api_STATUS_ARMGenerator gopter.Gen

// Api_STATUS_ARMGenerator returns a generator of Api_STATUS_ARM instances for property testing.
// We first initialize api_STATUS_ARMGenerator with a simplified generator based on the
// fields with primitive types then replacing it with a more complex one that also handles complex fields
// to ensure any cycles in the object graph properly terminate.
func Api_STATUS_ARMGenerator() gopter.Gen {
	if api_STATUS_ARMGenerator != nil {
		return api_STATUS_ARMGenerator
	}

	generators := make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForApi_STATUS_ARM(generators)
	api_STATUS_ARMGenerator = gen.Struct(reflect.TypeOf(Api_STATUS_ARM{}), generators)

	// The above call to gen.Struct() captures the map, so create a new one
	generators = make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForApi_STATUS_ARM(generators)
	AddRelatedPropertyGeneratorsForApi_STATUS_ARM(generators)
	api_STATUS_ARMGenerator = gen.Struct(reflect.TypeOf(Api_STATUS_ARM{}), generators)

	return api_STATUS_ARMGenerator
}

// AddIndependentPropertyGeneratorsForApi_STATUS_ARM is a factory method for creating gopter generators
func AddIndependentPropertyGeneratorsForApi_STATUS_ARM(gens map[string]gopter.Gen) {
	gens["Id"] = gen.PtrOf(gen.AlphaString())
	gens["Name"] = gen.PtrOf(gen.AlphaString())
	gens["Type"] = gen.PtrOf(gen.AlphaString())
}

// AddRelatedPropertyGeneratorsForApi_STATUS_ARM is a factory method for creating gopter generators
func AddRelatedPropertyGeneratorsForApi_STATUS_ARM(gens map[string]gopter.Gen) {
