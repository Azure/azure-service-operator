...
func Test_AuthorizationProvidersAuthorization_STATUS_ARM_WhenSerializedToJson_DeserializesAsEqual(t *testing.T) {
	t.Parallel()
	parameters := gopter.DefaultTestParameters()
	parameters.MinSuccessfulTests = 80
	parameters.MaxSize = 3
	properties := gopter.NewProperties(parameters)
	properties.Property(
		"Round trip of AuthorizationProvidersAuthorization_STATUS_ARM via JSON returns original",
		prop.ForAll(RunJSONSerializationTestForAuthorizationProvidersAuthorization_STATUS_ARM, AuthorizationProvidersAuthorization_STATUS_ARMGenerator()))
	properties.TestingRun(t, gopter.NewFormatedReporter(true, 240, os.Stdout))
}

// RunJSONSerializationTestForAuthorizationProvidersAuthorization_STATUS_ARM runs a test to see if a specific instance of AuthorizationProvidersAuthorization_STATUS_ARM round trips to JSON and back losslessly
func RunJSONSerializationTestForAuthorizationProvidersAuthorization_STATUS_ARM(subject AuthorizationProvidersAuthorization_STATUS_ARM) string {
	// Serialize to JSON
	bin, err := json.Marshal(subject)
	if err != nil {
		return err.Error()
	...
	// Deserialize back into memory
	var actual AuthorizationProvidersAuthorization_STATUS_ARM
	err = json.Unmarshal(bin, &actual)
	if err != nil {
		return err.Error()
	}
...
// Generator of AuthorizationProvidersAuthorization_STATUS_ARM instances for property testing - lazily instantiated by
// AuthorizationProvidersAuthorization_STATUS_ARMGenerator()
var authorizationProvidersAuthorization_STATUS_ARMGenerator gopter.Gen

// AuthorizationProvidersAuthorization_STATUS_ARMGenerator returns a generator of AuthorizationProvidersAuthorization_STATUS_ARM instances for property testing.
// We first initialize authorizationProvidersAuthorization_STATUS_ARMGenerator with a simplified generator based on the
// fields with primitive types then replacing it with a more complex one that also handles complex fields
// to ensure any cycles in the object graph properly terminate.
func AuthorizationProvidersAuthorization_STATUS_ARMGenerator() gopter.Gen {
	if authorizationProvidersAuthorization_STATUS_ARMGenerator != nil {
		return authorizationProvidersAuthorization_STATUS_ARMGenerator
	}

	generators := make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForAuthorizationProvidersAuthorization_STATUS_ARM(generators)
	authorizationProvidersAuthorization_STATUS_ARMGenerator = gen.Struct(reflect.TypeOf(AuthorizationProvidersAuthorization_STATUS_ARM{}), generators)

	// The above call to gen.Struct() captures the map, so create a new one
	generators = make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForAuthorizationProvidersAuthorization_STATUS_ARM(generators)
	AddRelatedPropertyGeneratorsForAuthorizationProvidersAuthorization_STATUS_ARM(generators)
	authorizationProvidersAuthorization_STATUS_ARMGenerator = gen.Struct(reflect.TypeOf(AuthorizationProvidersAuthorization_STATUS_ARM{}), generators)

	return authorizationProvidersAuthorization_STATUS_ARMGenerator
}

// AddIndependentPropertyGeneratorsForAuthorizationProvidersAuthorization_STATUS_ARM is a factory method for creating gopter generators
func AddIndependentPropertyGeneratorsForAuthorizationProvidersAuthorization_STATUS_ARM(gens map[string]gopter.Gen) {
	gens["Id"] = gen.PtrOf(gen.AlphaString())
	gens["Name"] = gen.PtrOf(gen.AlphaString())
	gens["Type"] = gen.PtrOf(gen.AlphaString())
}

// AddRelatedPropertyGeneratorsForAuthorizationProvidersAuthorization_STATUS_ARM is a factory method for creating gopter generators
func AddRelatedPropertyGeneratorsForAuthorizationProvidersAuthorization_STATUS_ARM(gens map[string]gopter.Gen) {
