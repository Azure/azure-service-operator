...
func Test_ServersFailoverGroup_STATUS_ARM_WhenSerializedToJson_DeserializesAsEqual(t *testing.T) {
	t.Parallel()
	parameters := gopter.DefaultTestParameters()
	parameters.MinSuccessfulTests = 80
	parameters.MaxSize = 3
	properties := gopter.NewProperties(parameters)
	properties.Property(
		"Round trip of ServersFailoverGroup_STATUS_ARM via JSON returns original",
		prop.ForAll(RunJSONSerializationTestForServersFailoverGroup_STATUS_ARM, ServersFailoverGroup_STATUS_ARMGenerator()))
	properties.TestingRun(t, gopter.NewFormatedReporter(true, 240, os.Stdout))
}

// RunJSONSerializationTestForServersFailoverGroup_STATUS_ARM runs a test to see if a specific instance of ServersFailoverGroup_STATUS_ARM round trips to JSON and back losslessly
func RunJSONSerializationTestForServersFailoverGroup_STATUS_ARM(subject ServersFailoverGroup_STATUS_ARM) string {
	// Serialize to JSON
	bin, err := json.Marshal(subject)
	if err != nil {
		return err.Error()
	...
	// Deserialize back into memory
	var actual ServersFailoverGroup_STATUS_ARM
	err = json.Unmarshal(bin, &actual)
	if err != nil {
		return err.Error()
	}
...
// Generator of ServersFailoverGroup_STATUS_ARM instances for property testing - lazily instantiated by
// ServersFailoverGroup_STATUS_ARMGenerator()
var serversFailoverGroup_STATUS_ARMGenerator gopter.Gen

// ServersFailoverGroup_STATUS_ARMGenerator returns a generator of ServersFailoverGroup_STATUS_ARM instances for property testing.
// We first initialize serversFailoverGroup_STATUS_ARMGenerator with a simplified generator based on the
// fields with primitive types then replacing it with a more complex one that also handles complex fields
// to ensure any cycles in the object graph properly terminate.
func ServersFailoverGroup_STATUS_ARMGenerator() gopter.Gen {
	if serversFailoverGroup_STATUS_ARMGenerator != nil {
		return serversFailoverGroup_STATUS_ARMGenerator
	}

	generators := make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForServersFailoverGroup_STATUS_ARM(generators)
	serversFailoverGroup_STATUS_ARMGenerator = gen.Struct(reflect.TypeOf(ServersFailoverGroup_STATUS_ARM{}), generators)

	// The above call to gen.Struct() captures the map, so create a new one
	generators = make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForServersFailoverGroup_STATUS_ARM(generators)
	AddRelatedPropertyGeneratorsForServersFailoverGroup_STATUS_ARM(generators)
	serversFailoverGroup_STATUS_ARMGenerator = gen.Struct(reflect.TypeOf(ServersFailoverGroup_STATUS_ARM{}), generators)

	return serversFailoverGroup_STATUS_ARMGenerator
}

// AddIndependentPropertyGeneratorsForServersFailoverGroup_STATUS_ARM is a factory method for creating gopter generators
func AddIndependentPropertyGeneratorsForServersFailoverGroup_STATUS_ARM(gens map[string]gopter.Gen) {
	gens["Id"] = gen.PtrOf(gen.AlphaString())
	gens["Location"] = gen.PtrOf(gen.AlphaString())
...
// AddRelatedPropertyGeneratorsForServersFailoverGroup_STATUS_ARM is a factory method for creating gopter generators
func AddRelatedPropertyGeneratorsForServersFailoverGroup_STATUS_ARM(gens map[string]gopter.Gen) {
