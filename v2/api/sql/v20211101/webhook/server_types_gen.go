...
// +kubebuilder:webhook:path=/mutate-sql-azure-com-v20211101-server,mutating=true,sideEffects=None,matchPolicy=Exact,failurePolicy=fail,groups=sql.azure.com,resources=servers,verbs=create;update,versions=v20211101,name=default.v20211101.servers.sql.azure.com,admissionReviewVersions=v1
...
func (server *Server) Default(ctx context.Context, obj runtime.Object) error {
	...
	if !ok {
		return fmt.Errorf("expected github.com/Azure/azure-service-operator/v2/api/sql/v20211101/Server, but got %T", obj)
	}
...
// +kubebuilder:webhook:path=/validate-sql-azure-com-v20211101-server,mutating=false,sideEffects=None,matchPolicy=Exact,failurePolicy=fail,groups=sql.azure.com,resources=servers,verbs=create;update,versions=v20211101,name=validate.v20211101.servers.sql.azure.com,admissionReviewVersions=v1
...
func (server *Server) ValidateCreate(ctx context.Context, resource runtime.Object) (admission.Warnings, error) {
	...
	if !ok {
		return nil, fmt.Errorf("expected github.com/Azure/azure-service-operator/v2/api/sql/v20211101/Server, but got %T", resource)
	}
...
func (server *Server) ValidateUpdate(ctx context.Context, oldResource runtime.Object, newResource runtime.Object) (admission.Warnings, error) {
	...
	if !ok {
		return nil, fmt.Errorf("expected github.com/Azure/azure-service-operator/v2/api/sql/v20211101/Server, but got %T", newResource)
	}
	oldObj, ok := oldResource.(*v20211101.Server)
	if !ok {
		return nil, fmt.Errorf("expected github.com/Azure/azure-service-operator/v2/api/sql/v20211101/Server, but got %T", oldResource)
	}
