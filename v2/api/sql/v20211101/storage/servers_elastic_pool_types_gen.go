...
func (pool *ServersElasticPool) ConvertFrom(hub conversion.Hub) error {
	...
	if !ok {
		return fmt.Errorf("expected sql/v20250101/storage/ServersElasticPool but received %T instead", hub)
	}
...
// Storage version of v20211101.ServersElasticPool_Spec
...
func (pool *ServersElasticPool_Spec) AssignProperties_From_ServersElasticPool_Spec(source *storage.ServersElasticPool_Spec) error {
	...
	// AutoPauseDelay
	if source.AutoPauseDelay != nil {
		propertyBag.Add("AutoPauseDelay", *source.AutoPauseDelay)
	} else {
		propertyBag.Remove("AutoPauseDelay")
	}
...
func (pool *ServersElasticPool_Spec) AssignProperties_To_ServersElasticPool_Spec(destination *storage.ServersElasticPool_Spec) error {
	...
	// AutoPauseDelay
	if propertyBag.Contains("AutoPauseDelay") {
		var autoPauseDelay int
		err := propertyBag.Pull("AutoPauseDelay", &autoPauseDelay)
		if err != nil {
			return eris.Wrap(err, "pulling 'AutoPauseDelay' from propertyBag")
		}

		destination.AutoPauseDelay = &autoPauseDelay
	} else {
		destination.AutoPauseDelay = nil
	}
...
// Storage version of v20211101.ServersElasticPool_STATUS
