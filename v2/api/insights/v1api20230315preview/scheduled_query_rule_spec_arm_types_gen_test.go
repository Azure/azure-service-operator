...
func AddRelatedPropertyGeneratorsForScheduledQueryRuleProperties_ARM(gens map[string]gopter.Gen) {
	...
	gens["RuleResolveConfiguration"] = gen.PtrOf(RuleResolveConfiguration_ARMGenerator())
}
...
func Test_RuleResolveConfiguration_ARM_WhenSerializedToJson_DeserializesAsEqual(t *testing.T) {
	t.Parallel()
	parameters := gopter.DefaultTestParameters()
	parameters.MinSuccessfulTests = 100
	parameters.MaxSize = 3
	properties := gopter.NewProperties(parameters)
	properties.Property(
		"Round trip of RuleResolveConfiguration_ARM via JSON returns original",
		prop.ForAll(RunJSONSerializationTestForRuleResolveConfiguration_ARM, RuleResolveConfiguration_ARMGenerator()))
	properties.TestingRun(t, gopter.NewFormatedReporter(true, 240, os.Stdout))
}

// RunJSONSerializationTestForRuleResolveConfiguration_ARM runs a test to see if a specific instance of RuleResolveConfiguration_ARM round trips to JSON and back losslessly
func RunJSONSerializationTestForRuleResolveConfiguration_ARM(subject RuleResolveConfiguration_ARM) string {
	// Serialize to JSON
	bin, err := json.Marshal(subject)
	if err != nil {
		return err.Error()
	...
	// Deserialize back into memory
	var actual RuleResolveConfiguration_ARM
	err = json.Unmarshal(bin, &actual)
	if err != nil {
		return err.Error()
	}
...
// Generator of RuleResolveConfiguration_ARM instances for property testing - lazily instantiated by
// RuleResolveConfiguration_ARMGenerator()
var ruleResolveConfiguration_ARMGenerator gopter.Gen

// RuleResolveConfiguration_ARMGenerator returns a generator of RuleResolveConfiguration_ARM instances for property testing.
func RuleResolveConfiguration_ARMGenerator() gopter.Gen {
	if ruleResolveConfiguration_ARMGenerator != nil {
		return ruleResolveConfiguration_ARMGenerator
	}

	generators := make(map[string]gopter.Gen)
	AddIndependentPropertyGeneratorsForRuleResolveConfiguration_ARM(generators)
	ruleResolveConfiguration_ARMGenerator = gen.Struct(reflect.TypeOf(RuleResolveConfiguration_ARM{}), generators)

	return ruleResolveConfiguration_ARMGenerator
}

// AddIndependentPropertyGeneratorsForRuleResolveConfiguration_ARM is a factory method for creating gopter generators
func AddIndependentPropertyGeneratorsForRuleResolveConfiguration_ARM(gens map[string]gopter.Gen) {
