...
type TypeConfiguration struct {
	...
	azureGeneratedConfigs       configurable[map[string]string]
	manualAzureGeneratedConfigs configurable[[]string]
...
const (
	...
	manualAzureGeneratedConfigsTag = "$manualAzureGeneratedConfigs" // A set of strings specifying which config maps are generated by Azure
...
func (tc *TypeConfiguration) SetAzureGeneratedConfigs(configMaps map[string]string) *TypeConfiguration {
...
func (tc *TypeConfiguration) AzureGeneratedConfigs() (map[string]string, error) {
	v, ok := tc.azureGeneratedConfigs.read()
	if !ok {
		msg := fmt.Sprintf("%s not specified for type %s", manualAzureGeneratedConfigsTag, tc.name)
		return nil, NewNotConfiguredError(msg)
	}
...
// SetManualAzureGeneratedConfigs sets the list of Azure Generated configmaps this type supports
func (tc *TypeConfiguration) SetManualAzureGeneratedConfigs(configMaps []string) *TypeConfiguration {
	tc.manualAzureGeneratedConfigs.write(configMaps)
	return tc
}

// ManualAzureGeneratedConfigs gets the list of Azure Generated config maps this type supports
func (tc *TypeConfiguration) ManualAzureGeneratedConfigs() ([]string, error) {
	v, ok := tc.manualAzureGeneratedConfigs.read()
	if !ok {
		msg := fmt.Sprintf("%s not specified for type %s", manualAzureGeneratedConfigsTag, tc.name)
		return nil, NewNotConfiguredError(msg)
	}
...
// VerifyManualAzureGeneratedConfigsConsumed returns an error if our configured azureGeneratedConfigs were not used,
// nil otherwise.
func (tc *TypeConfiguration) VerifyManualAzureGeneratedConfigsConsumed() error {
	if tc.manualAzureGeneratedConfigs.isUnconsumed() {
		return errors.Errorf("type %s: "+manualAzureGeneratedConfigsTag+": not consumed", tc.name)
	}
...
func (tc *TypeConfiguration) UnmarshalYAML(value *yaml.Node) error {
	...
	for i, c := range value.Content {
		...
		if strings.EqualFold(lastId, azureGeneratedConfigsTag) && c.Kind == yaml.MappingNode {
			...
			tc.SetAzureGeneratedConfigs(azureGeneratedConfigs)
			continue
		}
		...
		if strings.EqualFold(lastId, manualAzureGeneratedConfigsTag) && c.Kind == yaml.SequenceNode {
			...
			for _, content := range c.Content {
				if content.Kind == yaml.ScalarNode {
					...
					return errors.Errorf(
						"unexpected yam value for %s (line %d col %d)",
						manualAzureGeneratedConfigsTag,
						content.Line,
						content.Column)
				}
			}

			tc.SetManualAzureGeneratedConfigs(manualAzureGeneratedConfigs)
			continue
		}
