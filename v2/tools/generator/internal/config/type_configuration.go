...
type TypeConfiguration struct {
	...
	AzureNameFromConfig      configurable[bool]                                // Boolean indicating if the resource supports setting AzureName from a ConfigMap
	DefaultAzureName         configurable[bool]                                // Boolean indicating if the resource should automatically default AzureName
	ExportAs                 configurable[string]                              // String specifying the name to use for an exported type
...
const (
	...
	exportAsTag                 = "$exportAs"                 // String specifying the name to use for an exported type
...
func NewTypeConfiguration(name string) *TypeConfiguration {
	...
	return &TypeConfiguration{
		...
		AzureNameFromConfig:      makeConfigurable[bool](azureNameFromConfigTag, scope),
...
func (tc *TypeConfiguration) UnmarshalYAML(value *yaml.Node) error {
	...
	for i, c := range value.Content {
		...
		// $azureNameFromConfig: <bool>
		if strings.EqualFold(lastID, azureNameFromConfigTag) && c.Kind == yaml.ScalarNode {
			var azureNameFromConfig bool
			err := c.Decode(&azureNameFromConfig)
			if err != nil {
				return eris.Wrapf(err, "decoding %s", azureNameFromConfigTag)
			}

			tc.AzureNameFromConfig.Set(azureNameFromConfig)
			continue
		}
