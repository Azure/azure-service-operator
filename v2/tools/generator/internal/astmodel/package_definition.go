...
func (p *PackageDefinition) EmitDefinitions(outputDir string, generatedPackages map[PackageReference]*PackageDefinition) (int, error) {
	...
	if resources := FindResourceDefinitions(p.definitions); len(resources) > 0 {
		...
		// If package contains resources and is not Storage package, then we generate doc file for the package
		if !strings.HasSuffix(p.PackageName, StoragePackageSuffix) {
